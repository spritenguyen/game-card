...
export const InstructionModal: React.FC<Props> = ({ config, isOpen, onClose }) => {
    return (
        <AnimatePresence>
            {isOpen && (
                <div 
                ...
                >
                    <motion.div 
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="absolute inset-0 bg-black/90 backdrop-blur-md pointer-events-none" 
                    ></motion.div>
                        ...
                        <div className="p-6 border-b border-white/5 flex items-center justify-between shrink-0 bg-gradient-to-r from-cinematic-gold/10 to-transparent">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-lg bg-cinematic-gold/20 border border-cinematic-gold/40 flex items-center justify-center text-cinematic-gold">
                                    <Icon name="fa-book-open text-xl" className="fa-book-open text-xl" />
                                </div>
                                <div>
                                    <h3 className="text-xl font-serif text-white tracking-widest uppercase leading-none">{t(config.language || 'vi', 'instructions.modalTitle')}</h3>
                                    <p className="text-[9px] text-zinc-500 font-mono tracking-widest mt-1">{t(config.language || 'vi', 'instructions.versionString')}{APP_VERSION}</p>
                                </div>
                            </div>
                        ...
                        <div className="flex-1 p-6 overflow-y-auto no-scrollbar space-y-8">
                            ...
                            <section className="space-y-4">
                                <div className="flex items-center gap-2 border-b border-white/10 pb-2">
                                    <Icon name="fa-gamepad text-cinematic-cyan/80" className="fa-gamepad text-cinematic-cyan/80" />
                                    <h4 className="text-sm font-bold text-white font-mono uppercase tracking-widest">{t(config.language || 'vi', 'instructions.basicGuide')}</h4>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="bg-white/5 p-3 rounded-lg border border-white/10">
                                        <h5 className="text-[11px] font-bold text-cinematic-gold mb-1 font-mono uppercase"><Icon name="fa-radar mr-1" className="fa-radar mr-1" /> {t(config.language || 'vi', 'instructions.step1Title')}</h5>
                                        <p className="text-[10px] text-zinc-400 leading-relaxed">{t(config.language || 'vi', 'instructions.step1Desc')}</p>
                                    </div>
                                    <div className="bg-white/5 p-3 rounded-lg border border-white/10">
                                        <h5 className="text-[11px] font-bold text-cinematic-cyan mb-1 font-mono uppercase"><Icon name="fa-dna mr-1" className="fa-dna mr-1" /> {t(config.language || 'vi', 'instructions.step2Title')}</h5>
                                        <p className="text-[10px] text-zinc-400 leading-relaxed">{t(config.language || 'vi', 'instructions.step2Desc')}</p>
                                    </div>
                                    <div className="bg-white/5 p-3 rounded-lg border border-white/10">
                                        <h5 className="text-[11px] font-bold text-red-400 mb-1 font-mono uppercase"><Icon name="fa-crosshairs mr-1" className="fa-crosshairs mr-1" /> {t(config.language || 'vi', 'instructions.step3Title')}</h5>
                                        <p className="text-[10px] text-zinc-400 leading-relaxed">{t(config.language || 'vi', 'instructions.step3Desc')}</p>
                                    </div>
                                    <div className="bg-white/5 p-3 rounded-lg border border-white/10">
                                        <h5 className="text-[11px] font-bold text-purple-400 mb-1 font-mono uppercase"><Icon name="fa-shop mr-1" className="fa-shop mr-1" /> {t(config.language || 'vi', 'instructions.step4Title')}</h5>
                                        <p className="text-[10px] text-zinc-400 leading-relaxed">{t(config.language || 'vi', 'instructions.step4Desc')}</p>
                                    </div>
                                </div>
                            </section>
                            ...
                            <section className="space-y-3">
                                <div className="flex items-center gap-2">
                                    <div className="h-4 w-1 bg-gradient-to-b from-purple-500 to-orange-500 rounded-full"></div>
                                    <h4 className="text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-orange-500 font-mono uppercase tracking-widest">Version 2.4.0: Tactical Warfare Expansion</h4>
                                </div>
                                <div className="bg-cinematic-900/40 min-h-[50px] p-4 rounded-xl border border-purple-500/20 text-zinc-400 text-xs leading-relaxed space-y-2">
                                    <p><strong className="text-white">● Hệ thống Phân Lớp Vai Trò:</strong> Bổ sung phân nhóm chức năng (Tanker, DPS, Support). Thẻ thuộc vai trò <span className="text-cinematic-gold border border-cinematic-gold/30 px-1 rounded">Vanguard (Tiên Phong)</span> khi ra trận sẽ kích hoạt <span className="text-orange-400">cơ chế Đỡ Đòn (Taunt)</span>, thu hút phần lớn sát thương vật lý/tấn công thường từ kẻ địch.</p>
                                    <p><strong className="text-white">● Cơ Chế Xuyên Kháng / Phá Giáp:</strong> Black Market (Chợ Đen) chính thức mở khóa chức năng <span className="text-purple-400 font-bold">Reroll Gen (Cấy Nội Tại)</span>. Thẻ bài có thể mang Gen Phá Giáp để bỏ qua 20% giáp của địch. Đặc biệt, kỹ năng <span className="text-cyan-400 uppercase font-bold">Ultimate hệ Sét</span> nay bỏ qua 100% Giáp Vật Lý.</p>
                                    <p><strong className="text-white">● Lớp Giáp Ảo & Phản Sát Thương:</strong> Tương tác chiến thuật mới cho thẻ hệ <span className="text-amber-500">Earth</span> và <span className="text-teal-400">Tech</span>: Các hiệu ứng Hồi Máu (kể cả từ Thẻ Support hoặc Lệnh Emergency Repair) sẽ được chuyển hóa thành <span className="text-blue-300 font-bold">Lớp Khiên Ảo (Shield)</span>. Ngoài ra, thẻ Vanguard thuộc nhóm này nếu chịu sát thương sẽ <span className="text-orange-500 font-bold">phản xạ sát thương</span> dựa trên 50% DEF của thẻ.</p>
                                </div>
                            </section>
                            ...
                            <section className="space-y-3">
                                <div className="flex items-center gap-2">
                                    <div className="h-4 w-1 bg-gradient-to-b from-cinematic-gold to-red-500 rounded-full"></div>
                                    <h4 className="text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r from-cinematic-gold to-red-500 font-mono uppercase tracking-widest">Version 2.3.0: Cine-Tech Visual Overhaul & Architecture Refactor</h4>
                                </div>
                                <div className="bg-cinematic-900/40 min-h-[50px] p-4 rounded-xl border border-cinematic-gold/20 text-zinc-400 text-xs leading-relaxed space-y-1">
                                    <p><strong className="text-white">● Hệ thống Component Hóa Combat:</strong> Toàn bộ view chiến đấu (CombatView) đồ sộ đã được kiến trúc lại (Refactored) thành các component nhỏ gọn: <code>&lt;CombatHeader&gt;</code>, <code>&lt;CombatControls&gt;</code>, <code>&lt;CombatArena&gt;</code> và <code>&lt;CombatLogPanel&gt;</code>, giúp mã nguồn dễ dàng bảo trì mở rộng mà không làm thay đổi bất kỳ hiệu ứng đồ họa in-game có sẵn nào.</p>
                                    <p><strong className="text-white">● Combat Log Reborn:</strong> Giao diện Mission Log trong chiến đấu được thiết kế lại hoàn toàn. Áp dụng phong cách Cine-Tech với hiệu ứng Neon viền mờ ảo, nâng cấp phông chữ telemetry sinh động hơn để mang lại chất phim khoa học viễn tưởng.</p>
                                    <p><strong className="text-white">● Cinematic Ultimate:</strong> Khi thẻ kích hoạt Ultimate Move (Kỹ năng tối thượng), màn hình ngay lập tức hiển thị bộ thanh đen (Cinematic Letterbox), thu hẹp khung viền nhắm mục tiêu để tăng trọng lượng và chiều sâu vật lý cho đòn đánh.</p>
                                    <p><strong className="text-white">● Animation Impact:</strong> Cường hóa hệ thống Damage Popups, tinh chỉnh Quỹ đạo (Y-Axis) và tỉ lệ phóng to (Scale) khi xuất hiện bạo kích (Critical), tạo cảm giác uy lực thực sự mạnh mẽ (Punchy hits).</p>
                                </div>
                            </section>
                            ...
                            <section className="space-y-3">
                                <div className="flex items-center gap-2">
                                    <div className="h-4 w-1 bg-gradient-to-b from-cinematic-cyan to-cinematic-gold rounded-full"></div>
                                    <h4 className="text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r from-cinematic-cyan to-cinematic-gold font-mono uppercase tracking-widest">Version 0.1.0.12: Data Integrity Patch</h4>
                                </div>
                                <div className="bg-cinematic-900/40 min-h-[50px] p-4 rounded-xl border border-cinematic-gold/20 text-zinc-400 text-xs leading-relaxed space-y-1">
                                    <p><strong className="text-white">● Missions Hotfix:</strong> Sửa lỗi logic khởi tạo trạng thái khiến nhiệm vụ hiển thị màn hình trắng / biến mất nếu người dùng mở trò chơi lần đầu hoặc lỗi parse Storage.</p>
                                    <p><strong className="text-white">● Fallback UI:</strong> Bổ sung thêm hoạt ảnh và giao diện trống (Empty States) vào tất cả các màn hình, đặc biệt là tab Missions & Bounties (Expeditions, Storage).</p>
                                    <p><strong className="text-white">● Cleanup:</strong> Đã rà soát và làm sạch các đoạn code Hooks dư thừa ở cấp cao gây rò rỉ bộ nhớ/lỗi logic UI không mong muốn trong quá trình render ứng dụng.</p>
                                </div>
                            </section>
                            ...
                            <section className="space-y-3">
                                <div className="flex items-center gap-2">
                                    <div className="h-4 w-1 bg-gradient-to-b from-cinematic-cyan to-cinematic-gold rounded-full"></div>
                                    <h4 className="text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r from-cinematic-cyan to-cinematic-gold font-mono uppercase tracking-widest">Version 0.1.0.11: UX & API Routing Hotfix</h4>
                                </div>
                                <div className="bg-cinematic-900/40 min-h-[50px] p-4 rounded-xl border border-cinematic-gold/20 text-zinc-400 text-xs leading-relaxed space-y-1">
                                    <p><strong className="text-white">● API Silence Mode:</strong> Đã tắt hoàn toàn hộp thoại bật lên (nền vàng) của hệ thống thông báo trạng thái thay đổi API, giảm bớt sự khó chịu/rác UI khi đã có hệ thống monitor mini ở góc dưới bên trái màn hình.</p>
                                    <p><strong className="text-white">● Core Routing:</strong> Đóng/cắt cứng luồng Fallback của Gemini System. Khi tùy chọn "Gemini Custom" tắt, toàn bộ ứng dụng sẽ bị ép dùng 100% Pollinations làm Text Generator và không có ngoại lệ.</p>
                                    <p><strong className="text-white">● UI/UX Form Fix:</strong> Sửa lỗi cấu trúc vách ngăn (layer) của khung UI Cài Đặt (Settings) và Chọn Thẻ. Giờ đây việc bạn bôi đen Password Key hoặc kéo chuột vượt ra ngoài khung màn hình sẽ KHÔNG làm đóng đột ngột bảng tùy chọn nữa.</p>
                                </div>
                            </section>
                            ...
                            <section className="space-y-3">
                                <div className="flex items-center gap-2">
                                    <div className="h-4 w-1 bg-gradient-to-b from-cinematic-cyan to-cinematic-gold rounded-full"></div>
                                    <h4 className="text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r from-cinematic-cyan to-cinematic-gold font-mono uppercase tracking-widest">Version 0.1.0.10: Settings UI & AI Routing Hotfix</h4>
                                </div>
                                <div className="bg-cinematic-900/40 min-h-[50px] p-4 rounded-xl border border-cinematic-gold/20 text-zinc-400 text-xs leading-relaxed space-y-1">
                                    <p><strong className="text-white">● AI Pipeline Fix:</strong> Đưa Pollinations API trở lại làm luồng sinh văn bản/chỉ số (Text Generator) chính và mặc định. Gemini API giờ đây CHỈ được kích hoạt khi tính năng Custom Gemini được bật và người dùng cung cấp API Key hợp lệ nhằm tiết kiệm tối đa Quota Gemini theo ý định ban đầu.</p>
                                    <p><strong className="text-white">● UI Stability Patch:</strong> Khắc phục lỗi tự động đóng Modal (Giao diện cài đặt và UI Trích xuất/Lai tạo thẻ) khi người dùng tiến hành bôi đen văn bản trong các vùng nhập liệu (như API Key) rồi vô tình kéo thả chuột ra vùng nền ngoài (backdrop).</p>
                                </div>
                            </section>
                            ...
                            <section className="space-y-3">
                                <div className="flex items-center gap-2">
                                    <div className="h-4 w-1 bg-gradient-to-b from-cinematic-cyan to-cinematic-gold rounded-full"></div>
                                    <h4 className="text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r from-cinematic-cyan to-cinematic-gold font-mono uppercase tracking-widest">Version 0.1.0.9: AI Engine & Network Refactor</h4>
                                </div>
                                <div className="bg-cinematic-900/40 min-h-[50px] p-4 rounded-xl border border-cinematic-gold/20 text-zinc-400 text-xs leading-relaxed space-y-1">
                                    <p><strong className="text-white">● API Fallback Fix:</strong> Sửa lỗi hệ thống bỏ qua System Gemini Key khi tính năng Custom Gemini bị tắt. Gemini API sẽ được kích hoạt trở lại làm Text Generator mặc định và tốc độ cao nhất.</p>
                                    <p><strong className="text-white">● Proxy Route Upgrade:</strong> Cập nhật hệ thống định tuyến endpoint tự động cho Pollinations Backup API giúp giảm tải lỗi Timeout và lỗi "Dữ liệu trả về không phải JSON hợp lệ".</p>
                                    <p><strong className="text-white">● Image Render Async Patch:</strong> Tối ưu luồng promise của hệ thống Render ảnh AI, loại bỏ triệt để lỗi unhandled promise rejection gây kẹt Memory và làm khựng Background Tasks.</p>
                                </div>
                            </section>
                            ...
                            <section className="space-y-3">
                                <div className="flex items-center gap-2">
                                    <div className="h-4 w-1 bg-gradient-to-b from-cinematic-cyan to-cinematic-gold rounded-full"></div>
                                    <h4 className="text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r from-cinematic-cyan to-cinematic-gold font-mono uppercase tracking-widest">Version 0.1.0.8: Transaction Sync Hotfix</h4>
                                </div>
                                <div className="bg-cinematic-900/40 min-h-[50px] p-4 rounded-xl border border-cinematic-gold/20 text-zinc-400 text-xs leading-relaxed space-y-1">
                                    <p><strong className="text-white">● Fusion/Transaction Fix:</strong> Khắc phục lỗi hiển thị "Giao dịch Credit bị từ chối" ngay cả khi người chơi có đủ Data Credits trong quá trình lai tạo thẻ (Chimera). Việc thanh toán DC giờ đây sẽ diễn ra mượt mà và đồng bộ trên tất cả các tính năng dùng DC (Lai tạo, Thức Tỉnh, Chợ đen).</p>
                                </div>
                            </section>
                            ...
                            <div className="p-4 rounded-xl bg-cinematic-gold/5 border border-cinematic-gold/20 text-[10px] text-zinc-500 font-mono italic text-center">
                                {t(config.language || 'vi', 'instructions.note')}
                            </div>
                        </div>

                        {/* Footer */}
                        <div className="p-4 border-t border-white/5 shrink-0 bg-black/40">
                            <button onClick={onClose} className="w-full bg-cinematic-gold text-black py-3 rounded-xl font-bold tracking-widest uppercase text-xs shadow-[0_0_20px_rgba(255,184,0,0.3)] hover:scale-[1.02] transition-transform active:scale-95">{t(config.language || 'vi', 'instructions.btnClose')}</button>
                        </div>
                    </motion.div>
                </div>
            )}
