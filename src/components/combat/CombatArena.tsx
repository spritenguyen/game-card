  ...
  return (
    <motion.div
      ...
      style={{
        filter: hitStop
          ...
          : "none",
        transform: hitStop ? "scale(1.04)" : "scale(1)",
    ...
    >
      ...
      <motion.div
        ...
        className="fixed top-0 left-0 right-0 h-[8vh] bg-black z-[150] shadow-[0_10px_30px_rgba(0,0,0,0.8)] pointer-events-none"
      ></motion.div>
      <motion.div
        initial={{ y: "100%" }}
        animate={{ y: 0 }}
        exit={{ y: "100%" }}
        transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
        className="fixed bottom-0 left-0 right-0 h-[8vh] bg-black z-[150] shadow-[0_-10px_30px_rgba(0,0,0,0.8)] pointer-events-none"
      ></motion.div>

      {/* Film Grain / Noise Overlay */}
      <div className="fixed inset-0 z-[140] pointer-events-none opacity-[0.03] bg-[url('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAADIAAAAyBAMAAADsEZWCAAAAGFBMVEUAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA/c7OQAAAACHRSTlMAv/8Amf/uF9TGEVAAAAA6SURBVDjLY2AYBaNgFIyCUYAKGBgYDlABQzQOaGCIxwENDEmc0MBQxAkNDKWc0MBQzgkNDBWc0MBQzYkMAM9lB7C5P24rAAAAAElFTkSuQmCC')] repeat mix-blend-overlay"></div>
      ...
      <div className="w-full min-h-[100dvh] lg:h-screen lg:min-h-0 flex flex-col items-center justify-center py-12 sm:py-8 px-2 sm:px-4 relative">
        ...
        {/* Dynamic Boss Environment Layer */}
        {mounted && (boss as any).environment && (
          <div className="absolute inset-0 pointer-events-none overflow-hidden z-[5]">
             {((boss as any).environment.toLowerCase().includes("mưa") || (boss as any).environment.toLowerCase().includes("acid")) && (
                <div className="absolute inset-0 opacity-20 bg-[repeating-linear-gradient(170deg,transparent,transparent_10px,rgba(0,255,100,0.3)_10px,rgba(0,255,100,0.3)_11px)] animate-[pan_0.5s_linear_infinite]" />
             )}
             {((boss as any).environment.toLowerCase().includes("tuyết") || (boss as any).environment.toLowerCase().includes("lạnh")) && (
                <div className="absolute inset-0 opacity-20 bg-[radial-gradient(circle,_rgba(255,255,255,0.8)_2px,_transparent_2px)] animate-[slide-down_5s_linear_infinite]" style={{backgroundSize: '40px 40px'}} />
             )}
             {((boss as any).environment.toLowerCase().includes("điện") || (boss as any).environment.toLowerCase().includes("bão")) && (
                <div className="absolute inset-0 bg-blue-500/5 mix-blend-color-dodge animate-pulse" />
             )}
             {((boss as any).environment.toLowerCase().includes("dung nham") || (boss as any).environment.toLowerCase().includes("hỏa")) && (
                <div className="absolute inset-0 bg-orange-600/10 mix-blend-color-dodge animate-pulse shadow-[inset_0_0_100px_rgba(234,88,12,0.3)]" />
             )}
          </div>
        )}
        ...
        <div className="fixed top-2 sm:top-4 left-4 sm:left-6 right-4 sm:right-6 flex justify-between items-start z-[160] pointer-events-none">
          <div className="flex flex-col">
            <div className="text-[9px] font-black text-red-500 tracking-[0.3em] flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse border border-red-200"></div>
              {t('combat.liveFeed')}
            </div>
          ...
          <div className="text-right">
            ...
            {(boss as any).environment && (
              <div className="text-[8px] text-blue-400 font-mono font-bold mt-1 uppercase tracking-widest flex items-center justify-end gap-1">
                 <Icon name="fa-cloud-moon text-[10px]" /> ENV: {(boss as any).environment}
              </div>
            )}
            {(boss as any).level && (
              <div className="text-[8px] text-red-500 font-mono font-bold mt-0.5 uppercase tracking-widest flex items-center justify-end gap-1">
                 <Icon name="fa-skull text-[10px]" /> LVL: {(boss as any).level}
              </div>
            )}
          </div>
        </div>
