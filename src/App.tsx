...
export default function App() {
  ...
  useEffect(() => {
    // Add device classes for adaptive CSS gating
    if (device.isIOS) document.body.classList.add('is-ios');
    if (device.isAndroid) document.body.classList.add('is-android');
  ...
  const handleConfirm = (message: string, onConfirm: () => void) => {
    setDialogConfig({
      isOpen: true,
      title: t(config.language || 'vi', 'common.confirm'),
      message,
      type: "confirm",
      onConfirm,
    });
      ...
      <nav className={`absolute bottom-0 left-0 right-0 sm:relative z-40 bg-cinematic-900/90 backdrop-blur-2xl border-t sm:border-t-0 sm:border-r border-white/10 flex flex-row sm:flex-col justify-between sm:justify-start order-2 sm:order-1 shrink-0 h-[calc(72px+env(safe-area-inset-bottom))] sm:h-full transition-all duration-500 ease-in-out pb-[env(safe-area-inset-bottom)] sm:pb-0 ${
        ...
        <div className="hidden sm:flex items-center justify-center md:justify-start h-20 px-6 border-b border-white/5 bg-black/20">
          <div className="w-8 h-8 rounded shrink-0 bg-gradient-to-br from-white/10 to-transparent border border-white/10 flex items-center justify-center shadow-lg shadow-cinematic-cyan/5">
            <Icon name="fa-layer-group text-cinematic-cyan" className="fa-layer-group text-cinematic-cyan" />
          </div>
        ...
        <div className="flex sm:flex-col w-full h-full sm:h-auto px-0 sm:px-3 py-1 sm:py-6 gap-0 sm:gap-2 justify-between sm:justify-start items-center sm:items-stretch overflow-x-auto no-scrollbar scroll-smooth relative">
          ...
          {[
            { id: "campaign", icon: "BookOpen", label: t(config.language || 'vi', 'nav.campaign') },
            { id: "extract", icon: "Dna", label: t(config.language || 'vi', 'nav.extract') },
            { id: "forge", icon: "Cpu", label: t(config.language || 'vi', 'nav.forge') },
            { id: "combat", icon: "Crosshair", label: t(config.language || 'vi', 'nav.combat') },
            { id: "skills", icon: "Activity", label: t(config.language || 'vi', 'nav.skills') },
            { id: "missions", icon: "SatelliteDish", label: t(config.language || 'vi', 'nav.missions') },
            { id: "training", icon: "Dumbbell", label: t(config.language || 'vi', 'nav.training') },
            { id: "studio", icon: "Camera", label: config.language === 'en' ? 'Studio' : 'Studio' },
            { id: "breach", icon: "Terminal", label: config.language === 'en' ? 'Breach' : 'Bẻ Khóa' },
            { id: "phantasm", icon: "Mountain", label: config.language === 'en' ? 'Phantasm' : 'Tháp Ảo Ảnh' },
            { id: "clinic", icon: "Microchip", label: config.language === 'en' ? 'Clinic' : 'Cấy Ghép' },
            { id: "armory", icon: "ShieldHalved", label: config.language === 'en' ? 'Armory' : 'Kho Vũ Khí' },
            { id: "blackmarket", icon: "Store", label: t(config.language || 'vi', 'nav.blackmarket') },
            { id: "gallery", icon: "UserCircle", label: t(config.language || 'vi', 'nav.gallery') }
          ].map((btn) => (
            <button
              key={btn.id}
        ...
        <div className="hidden sm:flex flex-col mt-auto p-4 border-t border-white/5 space-y-2 bg-black/20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            <button
              ...
              title="Manual & Logs"
            >
              <Icon name="fa-book-open text-[10px]" className="fa-book-open text-[10px]" />
              <span className="hidden md:block text-[9px] font-mono uppercase tracking-[0.1em] font-bold">{t(config.language || 'vi', 'system.manual')}</span>
            </button>
            <button
              ...
              title="System Config"
            >
              <Icon name="fa-sliders text-[10px]" className="fa-sliders text-[10px]" />
              <span className="hidden md:block text-[9px] font-mono uppercase tracking-[0.1em] font-bold">{t(config.language || 'vi', 'system.config')}</span>
            </button>
          </div>
      ...
      <div className="relative z-20 flex-1 flex flex-col order-1 sm:order-2 h-screen w-full overflow-hidden pb-[calc(72px+env(safe-area-inset-bottom))] sm:pb-0">
        ...
        <header className="w-full h-14 sm:h-16 border-b border-white/5 bg-cinematic-900/60 backdrop-blur-md flex items-center justify-between px-2 sm:px-6 shrink-0 relative z-[40] overflow-hidden">
          ...
          <div className="hidden xs:block">
            <h2 className="text-[10px] sm:text-sm font-serif font-bold text-white tracking-[0.15em] sm:tracking-[0.3em] uppercase opacity-80 mt-1 truncate max-w-[80px] sm:max-w-none">
              {t(config.language || 'vi', `nav.titles.${activeTab}` as any)}
            </h2>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-4 flex-1 justify-end sm:flex-none">
             ...
             <div className="flex bg-black/40 border border-white/10 rounded-lg p-0.5 backdrop-blur-sm">
                <div className="flex items-center gap-1 px-1.5 sm:px-3 py-1 border-r border-white/10 hover:bg-white/5 rounded-l transition-colors cursor-help" title="Base Tickets">
                   <Icon name="fa-ticket text-cinematic-cyan/80 text-[10px] sm:text-xs" className="fa-ticket text-cinematic-cyan/80 text-[10px] sm:text-xs" />
                ...
                <div className="flex items-center gap-1 px-1.5 sm:px-3 py-1 border-r border-white/10 hover:bg-white/5 transition-colors cursor-help" title="Elite Tickets">
                   <Icon name="fa-ticket text-purple-400/80 text-[10px] sm:text-xs" className="fa-ticket text-purple-400/80 text-[10px] sm:text-xs" />
                ...
                <div className="flex xs:flex items-center gap-1 px-1.5 sm:px-3 py-1 hover:bg-white/5 rounded-r transition-colors cursor-help" title="Quantum Dust">
                   <Icon name="fa-galactic-senate text-amber-500/80 text-[10px] sm:text-xs" className="fa-galactic-senate text-amber-500/80 text-[10px] sm:text-xs" />
             ...
             <div className="sm:hidden flex gap-1 items-center">
               <button onClick={() => setShowInstructions(true)} className="w-7 h-7 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-zinc-400 active:bg-cinematic-cyan/20 active:text-cinematic-cyan transition-colors">
                 <Icon name="fa-circle-question text-xs" className="fa-circle-question text-xs" />
               </button>
               <button onClick={() => setShowSettings(true)} className="w-7 h-7 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-zinc-400 active:bg-cinematic-cyan/20 active:text-cinematic-cyan transition-colors">
                 <Icon name="fa-sliders text-xs" className="fa-sliders text-xs" />
               </button>
             </div>
          </div>
        </header>
        ...
        {activeTab === "combat" && (
          <CombatView
            ...
            onPhantasmWin={(newHps) => {
              setPhantasmProgress((prev) => ({
                floor: prev.floor + 1,
                cardsHp: newHps
              }));
            }}
                ...
                <h3 className="text-lg sm:text-2xl lg:text-3xl font-serif text-white tracking-widest uppercase flex items-center gap-2 sm:gap-3">
                  ...
                  {selectorTarget.startsWith("fusion") && (
                    <span className="text-[8px] sm:text-[10px] text-red-400 font-mono px-2 py-0.5 bg-red-950/50 rounded border border-red-900/50 flex align-middle">
                      <Icon name="fa-lock mr-1 mt-0.5" className="fa-lock mr-1 mt-0.5" /> UR LOCKED
                    </span>
                  )}
                </h3>
              </div>
                    ...
                    >
                      ...
                      {selected && (
                        <div className="absolute -top-2 -right-2 w-6 h-6 sm:w-8 sm:h-8 bg-cinematic-cyan text-black rounded-lg flex items-center justify-center shadow-[0_0_15px_var(--color-cinematic-cyan)] z-30 font-bold border-2 border-black">
                          <Icon name="fa-check text-[10px] sm:text-base" className="fa-check text-[10px] sm:text-base" />
                        </div>
                      )}
                    </div>
                  );
          ...
          <div className="relative z-10 w-full max-w-xl max-h-[85vh] overflow-y-auto no-scrollbar bg-black border border-white/10 rounded-2xl p-6 sm:p-8 shadow-[0_50px_100px_rgba(0,0,0,1)] ring-1 ring-white/5 animate-pop-in space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="text-xl sm:text-2xl font-serif text-white tracking-widest flex items-center gap-3">
                <Icon name="fa-microchip text-cinematic-cyan" className="fa-microchip text-cinematic-cyan" /> 
                SYSTEM CORE
              </h3>
            ...
            <div className="pt-4 sm:pt-6 mt-4 sm:mt-6 border-t border-red-900/30">
               ...
               >
                 <Icon name="fa-triangle-exclamation group-hover:animate-pulse" className="fa-triangle-exclamation group-hover:animate-pulse" /> Factory Reset
               </button>
            </div>
          </div>
        </div>
