...
interface Props {
  ...
  hasEnoughCurrency: (amount: number) => boolean;
  refreshQuests: () => void;
...
export const MissionsView: React.FC<Props> = ({ quests, expeditions, cards, inventory, updateQuestProgress, startExpedition, completeExpedition, claimExpedition, modifyCurrency, modifyInventory, setQuests, onAlert, hasEnoughCurrency, refreshQuests, refreshExpeditions, updateCard, config }) => {
    ...
    const handleClaimQuest = (quest: Quest) => {
        ...
        onAlert(t(config.language || 'vi', 'missions.bountyComplete'), t(config.language || 'vi', 'missions.bountyCompleteDesc').replace('{dc}', String(quest.rewardDC)));
    };

    const handleStartExpedition = (exp: Expedition) => {
        if (!selectedCardId) return onAlert(t(config.language || 'vi', 'missions.errorTitle'), t(config.language || 'vi', 'missions.errorSelectCard'));
        const card = cards.find(c => c.id === selectedCardId);
        if (!card) return;
        
        if (exp.requiredElement && card.element !== exp.requiredElement) return onAlert(t(config.language || 'vi', 'missions.errorTitle'), t(config.language || 'vi', 'missions.errorElement').replace('{el}', exp.requiredElement));
        if (exp.requiredFaction && card.faction !== exp.requiredFaction) return onAlert(t(config.language || 'vi', 'missions.errorTitle'), t(config.language || 'vi', 'missions.errorFaction').replace('{fac}', exp.requiredFaction));
    ...
    const handleCheckExpedition = (exp: Expedition) => {
        if (exp.status === 'ongoing' && exp.startTime) {
            ...
            if (timePassed >= targetTime) {
                ...
                if (exp.assignedCardId) {
                    const c = cards.find(x => x.id === exp.assignedCardId);
                    if (c) {
                        const affBonus = Math.floor(Math.random() * 3) + 2; // 2-4 affection
                        updateCard({ ...c, affection: (c.affection || 0) + affBonus });
                    }
                }
                
                onAlert(t(config.language || 'vi', 'missions.expeditionComplete'), t(config.language || 'vi', 'missions.expeditionCompleteDesc').replace('{dc}', String(exp.rewardDC)));
            } else {
                const leftTimer = Math.ceil((targetTime - timePassed)/1000/60);
                onAlert(t(config.language || 'vi', 'missions.expeditionOngoing'), t(config.language || 'vi', 'missions.expeditionOngoingDesc').replace('{time}', String(leftTimer)));
            }
        }
    }
            ...
            <div className="w-full max-w-6xl flex flex-col gap-6 mt-8 px-4 sm:px-6 relative z-10">
                ...
                <div className="flex justify-center mb-8 relative z-10 w-full max-w-2xl mx-auto">
                    <div className="flex bg-zinc-950/80 border border-white/5 rounded-full p-1.5 shadow-[inset_0_0_20px_rgba(0,0,0,0.8)] backdrop-blur-xl ring-1 ring-white/5 w-full">
                        <button onClick={() => setSubTab('quests')} className={`flex-1 relative z-10 py-3 rounded-full font-bold tracking-[0.2em] font-mono text-[10px] sm:text-xs uppercase transition-all duration-300 flex items-center justify-center gap-2 ${subTab==='quests'?'text-black':'text-zinc-500 hover:text-white'}`}>
                            <Icon name="fa-list-check" className="fa-list-check" /> {t(config.language || 'vi', 'missions.tabBounties')}
                        </button>
                        <button onClick={() => setSubTab('expeditions')} className={`flex-1 relative z-10 py-3 rounded-full font-bold tracking-[0.2em] font-mono text-[10px] sm:text-xs uppercase transition-all duration-300 flex items-center justify-center gap-2 ${subTab==='expeditions'?'text-black':'text-zinc-500 hover:text-white'}`}>
                            <Icon name="fa-map-location-dot" className="fa-map-location-dot" /> {t(config.language || 'vi', 'missions.tabExpeditions')}
                        </button>
                        <button onClick={() => setSubTab('storage')} className={`flex-1 relative z-10 py-3 rounded-full font-bold tracking-[0.2em] font-mono text-[10px] sm:text-xs uppercase transition-all duration-300 flex items-center justify-center gap-2 ${subTab==='storage'?'text-black':'text-zinc-500 hover:text-white'}`}>
                            <Icon name="fa-boxes-stacked" className="fa-boxes-stacked" /> {t(config.language || 'vi', 'missions.tabVault')}
                        </button>
                ...
                {subTab === 'quests' && (
                    <div className="flex flex-col gap-4">
                        <div className="flex justify-between items-center mb-2 px-2">
                            <div className="text-zinc-400 font-mono text-[10px] uppercase tracking-widest">
                                {t(config.language || 'vi', 'missions.reqToday')}
                            </div>
                            <button 
                                onClick={() => {
                                    if (hasEnoughCurrency(50)) {
                                        modifyCurrency(-50);
                                        refreshQuests();
                                        onAlert(t(config.language || 'vi', 'missions.systemTitle'), t(config.language || 'vi', 'system.bountiesRefreshed'));
                                    } else {
                                        onAlert(t(config.language || 'vi', 'missions.errorTitle'), t(config.language || 'vi', 'missions.notEnoughDc').replace('{dc}', '50'));
                                    }
                                }}
                                className="text-[10px] py-2 px-4 rounded-lg bg-zinc-900 border border-white/10 hover:border-cinematic-cyan text-zinc-300 hover:text-cinematic-cyan transition-all font-mono tracking-widest uppercase flex items-center gap-2"
                            >
                                <Icon name="fa-arrows-rotate" className="fa-arrows-rotate" />
                                {t(config.language || 'vi', 'missions.refreshDc').replace('{dc}', '50')}
                            </button>
                        </div>
                            ...
                            <div key={q.id} className={`bg-zinc-950/80 p-6 rounded-2xl border flex flex-col justify-between ${q.isCompleted && !q.isClaimed ? 'border-cinematic-cyan/30 ring-1 ring-cinematic-cyan/10 scale-[1.02]' : 'border-white/5 ring-1 ring-white/5 hover:border-white/20'} relative overflow-hidden backdrop-blur-xl shadow-[inset_0_0_50px_rgba(0,0,0,0.8)] transition-all duration-300 gap-6`}>
                                ...
                                <div className="flex-1 pl-3 relative z-10">
                                    ...
                                    <div className="flex flex-wrap gap-2 mt-auto">
                                        <div className="text-[8px] text-zinc-600 w-full mb-1 font-mono uppercase tracking-[0.3em]">{t(config.language || 'vi', 'missions.rewardPayload')}</div>
                                        <span className="text-[10px] bg-black/60 text-cinematic-gold px-3 py-1.5 rounded border border-cinematic-gold/20 font-mono uppercase flex items-center gap-2 shadow-inner"><Icon name="fa-coins" className="fa-coins" /> {q.rewardDC} DC</span>
                                        {q.rewardTickets?.map((tItem: any, idx: number) => (
                                            <span key={idx} className="text-[10px] bg-black/60 text-purple-400 px-3 py-1.5 rounded border border-purple-400/20 font-mono uppercase flex items-center gap-2 shadow-inner"><Icon name="fa-ticket" className="fa-ticket" /> {tItem.amount} {tItem.type} {t(config.language || 'vi', 'missions.ticketSuffix')}</span>
                                        ))}
                                    </div>
                                </div>
                                <div className="pl-3 relative z-10 w-full">
                                    {q.isClaimed ? (
                                        <button disabled className="w-full text-[10px] sm:text-xs px-4 py-3 bg-black/50 text-zinc-600 rounded-xl border border-white/5 font-mono font-bold tracking-[0.2em] uppercase cursor-not-allowed"><Icon name="fa-check mr-2" className="fa-check mr-2" /> {t(config.language || 'vi', 'missions.claimed')}</button>
                                    ) : q.isCompleted ? (
                                        <button onClick={() => handleClaimQuest(q)} className="w-full text-[10px] sm:text-xs font-bold px-4 py-3 bg-cinematic-cyan/10 hover:bg-cinematic-cyan hover:text-black text-cinematic-cyan transition-all font-mono tracking-[0.2em] uppercase rounded-xl border border-cinematic-cyan/30 shadow-[0_0_15px_rgba(0,243,255,0.2)] group relative overflow-hidden">
                                           ...
                                           <Icon name="fa-download mr-2" className="fa-download mr-2" /> {t(config.language || 'vi', 'missions.claimBounty')}
                                        </button>
                                    ) : (
                        ...
                        {quests.length === 0 && (
                            <div className="col-span-full py-20 flex flex-col items-center justify-center text-center bg-black/40 rounded-3xl border border-white/5 border-dashed shadow-inner">
                                 <Icon name="fa-list-check text-5xl text-zinc-700 w-24 h-24 bg-zinc-900 rounded-full flex items-center justify-center border border-white/5 mb-6 ring-1 ring-white/5 shadow-inner" className="fa-list-check text-5xl text-zinc-700 w-24 h-24 bg-zinc-900 rounded-full flex items-center justify-center border border-white/5 mb-6 ring-1 ring-white/5 shadow-inner" />
                                 <div className="text-white font-mono font-bold text-sm sm:text-base uppercase tracking-[0.2em] mb-2">{t(config.language || 'vi', 'missions.noBounties')}</div>
                                 <div className="text-zinc-500 font-mono text-[10px] sm:text-xs uppercase tracking-widest">{t(config.language || 'vi', 'missions.noBountiesDesc')}</div>
                            </div>
                        )}
                    </div>
                    </div>
                ...
                {subTab === 'expeditions' && (
                    <div className="flex flex-col gap-4">
                        <div className="flex justify-between items-center mb-2 px-2">
                            <div className="text-zinc-400 font-mono text-[10px] uppercase tracking-widest">
                                {t(config.language || 'vi', 'missions.exploreWorld')}
                            </div>
                            <button 
                                onClick={() => {
                                    if (hasEnoughCurrency(100)) {
                                        modifyCurrency(-100);
                                        refreshExpeditions();
                                        onAlert(t(config.language || 'vi', 'missions.systemTitle'), t(config.language || 'vi', 'system.expeditionsRefreshed'));
                                    } else {
                                        onAlert(t(config.language || 'vi', 'missions.errorTitle'), t(config.language || 'vi', 'missions.notEnoughDc').replace('{dc}', '100'));
                                    }
                                }}
                                className="text-[10px] py-2 px-4 rounded-lg bg-zinc-900 border border-white/10 hover:border-cinematic-gold text-zinc-300 hover:text-cinematic-gold transition-all font-mono tracking-widest uppercase flex items-center gap-2"
                            >
                                <Icon name="fa-arrows-rotate" className="fa-arrows-rotate" />
                                {t(config.language || 'vi', 'missions.refreshDc').replace('{dc}', '100')}
                            </button>
                        </div>
                            ...
                            <div key={exp.id} className="bg-zinc-950/80 p-6 sm:p-8 rounded-3xl border border-cinematic-gold/10 ring-1 ring-cinematic-gold/5 relative overflow-hidden backdrop-blur-xl shadow-[inset_0_0_50px_rgba(0,0,0,0.8)]">
                                ...
                                {selectedExp === exp.id ? (
                                    <div className="flex flex-col h-full justify-between animate-fade-in relative z-10">
                                        <div>
                                            <h4 className="text-cinematic-gold font-serif text-sm sm:text-base uppercase tracking-[0.3em] mb-4 flex items-center gap-3"><Icon name="fa-street-view text-xl" className="fa-street-view text-xl" /> {t(config.language || 'vi', 'missions.assignOperative')}</h4>
                                            <p className="text-[10px] sm:text-xs text-zinc-400 mb-6 font-mono uppercase tracking-[0.2em] bg-black/60 px-4 py-2.5 rounded-xl border border-white/5 inline-flex items-center gap-2 shadow-inner"><Icon name="fa-triangle-exclamation text-cinematic-gold" className="fa-triangle-exclamation text-cinematic-gold" /> {t(config.language || 'vi', 'missions.reqs')}: <span className="text-white font-bold">{exp.requiredElement ? `${exp.requiredElement} Class` : exp.requiredFaction ? `${exp.requiredFaction} Faction` : t(config.language || 'vi', 'missions.none')}</span></p>
                                            
                                            <div className="relative">
                                                <Icon name="fa-users absolute left-4 top-1/2 transform -translate-y-1/2 text-cinematic-gold/50 z-10 text-lg" className="fa-users absolute left-4 top-1/2 transform -translate-y-1/2 text-cinematic-gold/50 z-10 text-lg" />
                                                <select 
                                                ...
                                                >
                                                    <option value="">{t(config.language || 'vi', 'missions.selectOperative')}</option>
                                                ...
                                                <Icon name="fa-caret-down absolute right-4 top-1/2 transform -translate-y-1/2 text-zinc-500 pointer-events-none" className="fa-caret-down absolute right-4 top-1/2 transform -translate-y-1/2 text-zinc-500 pointer-events-none" />
                                            </div>
                                        </div>
                                        <div className="flex gap-4 mt-8">
                                            <button onClick={() => setSelectedExp(null)} className="flex-1 px-4 py-3 sm:py-4 text-[10px] sm:text-xs font-mono font-bold tracking-[0.2em] uppercase border border-white/10 hover:border-white/30 rounded-xl text-zinc-400 hover:text-white bg-black/40 hover:bg-white/5 transition-all">{t(config.language || 'vi', 'missions.abort')}</button>
                                            <button onClick={() => handleStartExpedition(exp)} className="flex-1 px-4 py-3 sm:py-4 text-[10px] sm:text-xs font-mono font-bold tracking-[0.2em] uppercase bg-cinematic-gold/10 text-cinematic-gold border border-cinematic-gold/30 rounded-xl hover:bg-cinematic-gold hover:text-black transition-all shadow-[0_0_20px_rgba(255,186,8,0.15)] relative overflow-hidden group">
                                                 <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent translate-x-[-100%] group-hover:duration-1000 group-hover:translate-x-[100%] transition-transform"></div>
                                                 {t(config.language || 'vi', 'missions.dispatch')}
                                            </button>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="flex flex-col h-full justify-between gap-6 relative z-10">
                                        <div>
                                            <div className="flex justify-between items-start mb-4 gap-4">
                                                ...
                                                <span className="text-[10px] bg-black/80 border border-white/10 text-cinematic-gold px-3 py-1.5 rounded-lg font-mono font-bold flex items-center gap-2 shadow-inner whitespace-nowrap"><Icon name="fa-clock" className="fa-clock" /> {exp.durationMinutes}M</span>
                                            </div>
                                            <p className="text-[10px] sm:text-xs text-zinc-400 font-mono tracking-[0.1em] leading-relaxed uppercase">{exp.description}</p>
                                        </div>
                                        <div className="mt-auto">
                                            <div className="text-[9px] sm:text-[10px] text-zinc-600 font-mono uppercase tracking-[0.3em] mb-3 flex items-center gap-2"><Icon name="fa-gift" className="fa-gift" /> {t(config.language || 'vi', 'missions.payload')}</div>
                                            <div className="flex flex-wrap gap-2 sm:gap-3 mb-6 bg-black/40 p-3 sm:p-4 rounded-xl border border-white/5 shadow-inner">
                                                <span className="text-[10px] sm:text-xs bg-cinematic-gold/10 text-cinematic-gold px-3 py-1.5 rounded-lg border border-cinematic-gold/30 font-mono font-bold flex items-center gap-2"><Icon name="fa-coins" className="fa-coins" /> {exp.rewardDC} DC</span>
                                                {exp.rewardMaterials?.map((m: any, idx: number) => (
                                                    <span key={idx} className="text-[10px] sm:text-xs bg-cinematic-cyan/10 text-cinematic-cyan px-3 py-1.5 rounded-lg border border-cinematic-cyan/30 font-mono font-bold flex items-center gap-2"><Icon name="fa-cube" className="fa-cube" /> {m.amount} {m.item}</span>
                                                ))}
                                            </div>
                                            {exp.status === 'idle' && (
                                                <button onClick={() => setSelectedExp(exp.id)} className="w-full py-4 sm:py-5 text-[10px] sm:text-xs font-mono font-bold tracking-[0.3em] uppercase bg-black hover:bg-cinematic-gold/10 border border-white/10 hover:border-cinematic-gold/30 text-white hover:text-cinematic-gold rounded-xl transition-all flex items-center justify-center gap-3 group">
                                                    <Icon name="fa-location-arrow group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" className="fa-location-arrow group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" /> {t(config.language || 'vi', 'missions.planDispatch')}
                                                </button>
                                            )}
                                            {exp.status === 'ongoing' && (
                                                <button onClick={() => handleCheckExpedition(exp)} className="w-full py-4 sm:py-5 text-[10px] sm:text-xs font-mono font-bold tracking-[0.3em] uppercase bg-cinematic-cyan/10 hover:bg-cinematic-cyan border border-cinematic-cyan/30 hover:border-cinematic-cyan text-cinematic-cyan hover:text-black rounded-xl shadow-[0_0_20px_rgba(0,243,255,0.15)] transition-all flex items-center justify-center gap-3 relative overflow-hidden group">
                                                    <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent translate-x-[-100%] group-hover:duration-1000 group-hover:translate-x-[100%] transition-transform"></div>
                                                    <Icon name="fa-satellite-dish animate-pulse" className="fa-satellite-dish animate-pulse" /> {t(config.language || 'vi', 'missions.checkStatus')}
                                                </button>
                                            )}
                                        </div>
                                    </div>
                        ...
                        {expeditions.length === 0 && (
                            <div className="col-span-full py-20 flex flex-col items-center justify-center text-center bg-black/40 rounded-3xl border border-white/5 border-dashed shadow-inner">
                                 <Icon name="fa-map-location-dot text-5xl text-zinc-700 w-24 h-24 bg-zinc-900 rounded-full flex items-center justify-center border border-white/5 mb-6 ring-1 ring-white/5 shadow-inner" className="fa-map-location-dot text-5xl text-zinc-700 w-24 h-24 bg-zinc-900 rounded-full flex items-center justify-center border border-white/5 mb-6 ring-1 ring-white/5 shadow-inner" />
                                 <div className="text-white font-mono font-bold text-sm sm:text-base uppercase tracking-[0.2em] mb-2">{t(config.language || 'vi', 'missions.noExpeditions')}</div>
                                 <div className="text-zinc-500 font-mono text-[10px] sm:text-xs uppercase tracking-widest">{t(config.language || 'vi', 'missions.noExpeditionsDesc')}</div>
                            </div>
                        )}
                    </div>
                    </div>
                    ...
                    <div className="bg-zinc-950/80 p-8 sm:p-12 rounded-3xl border border-white/5 ring-1 ring-white/5 backdrop-blur-xl shadow-[inset_0_0_100px_rgba(0,0,0,0.8),0_0_40px_rgba(192,132,252,0.05)] relative overflow-hidden">
                        ...
                        <div className="flex flex-col items-center mb-10 pb-8 border-b border-white/5 relative z-10">
                             <div className="w-16 h-16 rounded-2xl bg-purple-500/10 border border-purple-500/30 flex items-center justify-center mb-4 shadow-[0_0_30px_rgba(168,85,247,0.2)]">
                                 <Icon name="fa-boxes-stacked text-3xl text-purple-400" className="fa-boxes-stacked text-3xl text-purple-400" />
                             </div>
                             <h3 className="text-white text-xl sm:text-2xl font-black font-mono uppercase tracking-[0.3em] drop-shadow-md">{t(config.language || 'vi', 'missions.materialVault')}</h3>
                             <p className="text-[10px] sm:text-xs text-zinc-500 font-mono mt-2 uppercase tracking-[0.2em]">{t(config.language || 'vi', 'missions.vaultDesc')}</p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6 relative z-10">
                            {Object.entries(inventory.materials || {}).filter(([_, amount]) => Number(amount) > 0).map(([item, amount]) => (
                                <div key={item} className="bg-black/80 border border-white/5 hover:border-purple-400/30 rounded-2xl p-5 flex flex-col items-center justify-center text-center transition-all group shadow-inner relative overflow-hidden ring-1 ring-white/5 hover:ring-purple-400/20 hover:scale-105 duration-300">
                                    ...
                                    <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl bg-zinc-900/80 flex items-center justify-center border border-white/10 group-hover:border-purple-400/50 text-zinc-500 group-hover:text-purple-400 shadow-[inset_0_0_15px_rgba(0,0,0,0.8)] mb-4 transition-all relative z-10">
                                        <Icon name="fa-flask text-2xl sm:text-3xl drop-shadow-[0_0_10px_currentColor]" className="fa-flask text-2xl sm:text-3xl drop-shadow-[0_0_10px_currentColor]" />
                                    </div>
                            ...
                            {Object.keys(inventory.materials || {}).length === 0 && (
                                <div className="col-span-full py-20 flex flex-col items-center justify-center text-center bg-black/40 rounded-3xl border border-white/5 border-dashed shadow-inner">
                                     <Icon name="fa-box-open text-5xl text-zinc-700 w-24 h-24 bg-zinc-900 rounded-full flex items-center justify-center border border-white/5 mb-6 ring-1 ring-white/5 shadow-inner" className="fa-box-open text-5xl text-zinc-700 w-24 h-24 bg-zinc-900 rounded-full flex items-center justify-center border border-white/5 mb-6 ring-1 ring-white/5 shadow-inner" />
                                     <div className="text-white font-mono font-bold text-sm sm:text-base uppercase tracking-[0.2em] mb-2">{t(config.language || 'vi', 'missions.vaultEmpty')}</div>
                                     <div className="text-zinc-500 font-mono text-[10px] sm:text-xs uppercase tracking-widest">{t(config.language || 'vi', 'missions.vaultEmptyDesc')}</div>
                                </div>
                            )}
                        </div>
                    </div>
