...
const CONCEPTS = [
  { id: 'vogue_editorial', label: 'Vogue Editorial', description: 'High-end magazine photoshoot, dramatic lighting', icon: 'fa-camera' },
  { id: 'met_gala', label: 'Met-Gala Extravaganza', description: 'Avant-garde, extremely detailed and luxurious dresses/suits', icon: 'fa-gem' },
  { id: 'cyber_gothic', label: 'Cyber-Gothic', description: 'Dark, edgy, leather, cybernetic accessories', icon: 'fa-skull' },
  { id: 'streetwear', label: 'High-End Streetwear', description: 'Urban fashion, oversized, tech-wear elements', icon: 'fa-shoe-prints' },
  { id: 'haute_couture', label: 'Haute Couture', description: 'Custom-fitted, exclusive fashion, artistic expression', icon: 'fa-crown' },
  { id: 'y2k_retro', label: 'Y2K Retro Futurism', description: 'Glossy, metallic, vibrant 2000s tech aesthetics', icon: 'fa-compact-disc' },
];
...
export const StudioView: React.FC<Props> = ({ config, currency, modifyCurrency, inventory, modifyInventory, cards, updateCard, onAlert, isGlobalProcessing, setGlobalProcessing }) => {
  ...
  const openModelSelection = () => {
    ...
    if ((inventory?.quantumDust || 0) < PHOTOSHOOT_COST) return onAlert(isEn ? 'Error' : 'Lỗi', isEn ? 'Not enough Quantum Dust.' : `Không đủ ${PHOTOSHOOT_COST} Quantum Dust.`);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto h-full flex flex-col pt-24 sm:pt-6">
      <div className="flex flex-col mb-8 relative z-10 w-full text-center">
         ...
         <p className="text-[10px] sm:text-xs text-zinc-400 font-mono mt-3 uppercase tracking-[0.3em]">{isEn ? 'Haute Couture & Lenses' : 'Nhiếp Ảnh Thời Trang & Ống Kính'}</p>
      </div>
      
      <div className="flex overflow-x-auto no-scrollbar gap-2 sm:gap-6 border-b border-white/5 pb-0 mb-6 px-2 justify-center">
          <button onClick={() => setActiveTab('photoshoot')} className={`whitespace-nowrap text-[10px] sm:text-xs tracking-[0.2em] font-mono uppercase px-4 py-3 sm:py-4 transition-all relative ${activeTab==='photoshoot'?'text-pink-400':'text-zinc-500 hover:text-zinc-300'}`}>
              <Icon name="fa-camera mr-2" /> Photoshoot
              {activeTab === 'photoshoot' && <div className="absolute bottom-0 left-0 w-full h-[2px] bg-pink-500 shadow-[0_0_10px_rgba(236,72,153,0.5)]"></div>}
          </button>
          <button onClick={() => setActiveTab('lenses')} className={`whitespace-nowrap text-[10px] sm:text-xs tracking-[0.2em] font-mono uppercase px-4 py-3 sm:py-4 transition-all relative ${activeTab==='lenses'?'text-cinematic-cyan':'text-zinc-500 hover:text-zinc-300'}`}>
              <Icon name="fa-camera-retro mr-2" /> Camera Lenses
              {activeTab === 'lenses' && <div className="absolute bottom-0 left-0 w-full h-[2px] bg-cinematic-cyan shadow-[0_0_10px_rgba(0,243,255,0.5)]"></div>}
          </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 z-10">
        <div className="space-y-6">
            <div className="bg-black/60 border border-white/10 rounded-2xl p-6 shadow-xl backdrop-blur-md">
                 <h4 className="text-white font-mono text-sm uppercase tracking-widest mb-4 flex items-center gap-2"><Icon name="fa-users" /> {isEn ? '1. SELECT OPERATIVE' : '1. CHỌN ĐẶC VỤ'}</h4>
                 <div className="flex flex-col gap-2 max-h-[300px] sm:max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
                     {cards.map(c => (
                         <div 
                           key={c.id} 
                           onClick={() => setSelectedCardId(c.id)}
                           className={`cursor-pointer relative rounded-xl border p-3 flex items-center gap-3 transition-all ${selectedCardId === c.id ? 'border-pink-500 bg-pink-500/10 shadow-[0_0_10px_rgba(236,72,153,0.15)] ring-1 ring-pink-500/50' : 'border-white/5 bg-black/40 hover:border-white/20 hover:bg-black/60'}`}
                         >
                             ...
                             {c.equippedLens && (
                                <div className="text-[10px] sm:text-[10px] text-cinematic-cyan border border-cinematic-cyan/30 px-2 py-0.5 rounded font-bold shrink-0 ml-2 shadow-[0_0_5px_rgba(0,243,255,0.2)]">
                                    <Icon name="fa-camera-retro" /> LENS
                                </div>
                             )}
                         </div>
                     ))}
            ...
            <div className="bg-black/60 border border-white/10 rounded-2xl p-6 shadow-xl backdrop-blur-md animate-fade-in">
                     ...
                     {CONCEPTS.map(concept => (
                         ...
                         >
                             <div className="flex bg-black/40 w-8 h-8 rounded-full items-center justify-center mb-3 border border-white/5">
                                 <Icon name={`${concept.icon} z-10 relative ${selectedConcept === concept.id ? 'text-pink-400' : 'text-zinc-400'}`} />
                             </div>
            ...
            {activeTab === 'lenses' && (
            <div className="bg-black/60 border border-white/10 rounded-2xl p-6 shadow-xl backdrop-blur-md animate-fade-in">
                 <h4 className="text-cinematic-cyan font-mono text-sm uppercase tracking-widest mb-4 flex items-center gap-2"><Icon name="fa-camera-retro" /> {isEn ? '2. SELECT LENS / FILTER' : '2. CHỌN ỐNG KÍNH / KÍNH LỌC'}</h4>
                 <div className="text-[10px] sm:text-xs text-zinc-400 font-mono mt-2 uppercase tracking-widest mb-6">Trang bị Lens sẽ tăng chỉ số Combat và tự động thêm prompt ảnh hưởng vào Ảnh Render ở Studio!</div>
                 <div className="grid grid-cols-1 gap-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                     {LENSES.map(lens => (
                         <div 
                           key={lens.id}
                           onClick={() => setSelectedLens(lens.id)}
                           className={`cursor-pointer rounded-xl border p-4 flex gap-4 items-center transition-all ${selectedCard?.equippedLens === lens.id ? 'border-cinematic-cyan bg-cinematic-cyan/10 shadow-[0_0_15px_rgba(0,243,255,0.3)]' : selectedLens === lens.id ? 'border-zinc-400 bg-zinc-800/50' : 'border-white/5 bg-zinc-900/50 hover:border-white/20'}`}
                         >
                             <div className="flex bg-black/40 w-12 h-12 rounded-full items-center justify-center border border-white/5 shrink-0">
                                 <Icon name={`${lens.icon} text-lg z-10 relative ${selectedCard?.equippedLens === lens.id ? 'text-cinematic-cyan' : 'text-zinc-400'}`} />
                             </div>
                             <div className="flex-1">
                                <div className={`text-xs font-bold font-mono tracking-wider uppercase mb-1 ${selectedCard?.equippedLens === lens.id ? 'text-cinematic-cyan' : 'text-white'}`}>{lens.label}</div>
                                <div className="text-[10px] text-zinc-500 leading-relaxed font-mono">{lens.description}</div>
                             </div>
                             <button
                               disabled={!selectedCardId}
                               onClick={(e) => { e.stopPropagation(); handleEquipLens(lens.id); }}
                               className={`px-3 py-1.5 rounded-lg text-[10px] font-mono tracking-widest uppercase border ${selectedCard?.equippedLens === lens.id ? 'bg-cinematic-cyan text-black border-cinematic-cyan font-bold' : 'bg-transparent text-white border-white/20 hover:bg-white/10 disabled:opacity-30'}`}
                             >
                               {selectedCard?.equippedLens === lens.id ? 'EQUIPPED' : 'EQUIP'}
                             </button>
                         </div>
                     ))}
                 </div>
        ...
        {activeTab === 'photoshoot' && (
        <div className="bg-black/80 border border-white/10 rounded-3xl p-6 relative flex flex-col overflow-hidden shadow-2xl ring-1 ring-white/5 animate-fade-in">
             ...
             <div className="flex items-center justify-between mb-4 bg-black/40 p-4 rounded-xl border border-white/5">
                 <div>
                     ...
                     <div className="text-sm font-bold text-amber-500 flex items-center gap-2"><Icon name="fa-galactic-senate" /> {PHOTOSHOOT_COST} DUST</div>
                 </div>
                 {selectedCard && (
