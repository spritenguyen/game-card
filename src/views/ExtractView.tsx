    ...
    <div className="w-full flex flex-col items-center animate-fade-in pb-12">
      <div className="w-full max-w-3xl mb-8 bg-zinc-950/80 border border-cinematic-cyan/20 ring-1 ring-cinematic-cyan/10 rounded-3xl p-6 sm:p-10 flex flex-col gap-6 sm:gap-8 shadow-[inset_0_0_100px_rgba(0,0,0,0.8),0_0_40px_rgba(0,243,255,0.05)] relative mt-8 backdrop-blur-xl overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row items-center sm:justify-between gap-6 border-b border-cinematic-cyan/10 pb-6 relative z-10">
            <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-xl bg-cinematic-cyan/10 border border-cinematic-cyan/30 flex items-center justify-center text-cinematic-cyan shadow-[0_0_20px_rgba(0,243,255,0.2)]">
                    <Icon name="fa-dna text-2xl animate-pulse" className="fa-dna text-2xl animate-pulse" />
                </div>
                <div>
                    <h2 className="text-xl sm:text-2xl font-black font-mono text-transparent bg-clip-text bg-gradient-to-r from-white to-cinematic-cyan tracking-[0.3em] uppercase leading-none">
                        {t(config.language || 'vi', 'extract.title')}
                    </h2>
                    <p className="text-[10px] sm:text-xs text-cinematic-cyan/60 font-mono tracking-widest mt-1.5 uppercase drop-shadow-[0_0_5px_rgba(0,243,255,0.3)]">
                        {t(config.language || 'vi', 'extract.subtitle')}
                    </p>
                </div>
            </div>
            ...
            <div className="bg-black/60 border border-cinematic-cyan/30 text-cinematic-cyan px-4 py-2 sm:py-3 rounded-xl text-[10px] sm:text-xs font-mono tracking-[0.2em] flex items-center gap-3 shadow-[inset_0_0_15px_rgba(0,0,0,0.5),0_0_15px_rgba(0,243,255,0.1)] uppercase w-full sm:w-auto justify-center">
                <Icon name="fa-crosshairs animate-pulse" className="fa-crosshairs animate-pulse" />
                <span>{t(config.language || 'vi', 'extract.pity')}: <strong className="text-white text-sm">{pityCounter}</strong> / 90</span>
                {pityCounter >= 50 && <span className="text-yellow-400 font-bold ml-1 border-l border-white/20 pl-3">{t(config.language || 'vi', 'extract.highTide')}</span>}
                {pityCounter >= 89 && <span className="text-purple-400 font-bold ml-1 border-l border-white/20 pl-3 animate-pulse">{t(config.language || 'vi', 'extract.guaranteed')}</span>}
            </div>
        </div>
        ...
        <div className="relative flex-1 flex flex-col mt-2 z-10">
            ...
            <label className="text-[9px] sm:text-[10px] font-mono text-zinc-500 uppercase tracking-[0.3em] mb-3 pl-1 flex items-center gap-2"><Icon name="fa-terminal text-zinc-700" className="fa-terminal text-zinc-700" /> {t(config.language || 'vi', 'extract.inputLabel')}</label>
            <div className="relative">
                <Icon name="fa-fingerprint absolute left-4 top-1/2 transform -translate-y-1/2 text-cinematic-cyan/50 text-lg" className="fa-fingerprint absolute left-4 top-1/2 transform -translate-y-1/2 text-cinematic-cyan/50 text-lg" />
                <input 
                    type="text" 
                    value={query}
        ...
        {availableCores.length > 0 && (
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 bg-black/40 p-4 sm:p-5 rounded-2xl border border-cinematic-gold/10 ring-1 ring-cinematic-gold/5 relative overflow-hidden z-10 shadow-inner">
                ...
                <span className="text-[10px] sm:text-xs text-zinc-400 uppercase font-mono tracking-widest pl-3 flex items-center shrink-0">
                    <Icon name="fa-flask-vial text-cinematic-gold mr-2 text-lg" className="fa-flask-vial text-cinematic-gold mr-2 text-lg" /> {t(config.language || 'vi', 'extract.catalyst')}
                </span>
                <div className="relative flex-1 w-full">
                    ...
                    >
                        <option value="">{t(config.language || 'vi', 'extract.noAdditive')}</option>
                        {availableCores.map(core => (
                            <option key={core} value={core}>{core} (Qty: {inventory.materials[core]})</option>
                        ))}
                    </select>
                    <Icon name="fa-caret-down absolute right-4 top-1/2 transform -translate-y-1/2 text-zinc-600 pointer-events-none" className="fa-caret-down absolute right-4 top-1/2 transform -translate-y-1/2 text-zinc-600 pointer-events-none" />
                </div>
                {selectedCore && (
                    <span className="text-[10px] text-cinematic-gold border border-cinematic-gold/30 px-3 py-2 rounded-lg bg-cinematic-gold/5 whitespace-nowrap font-mono shrink-0 shadow-inner tracking-widest uppercase">
                        {t(config.language || 'vi', 'extract.bias')} {selectedCore.replace(' Core', '')}
                    </span>
                )}
            </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-6 z-10 w-full max-w-[600px] mx-auto">
            ...
            >
                ...
                <div className="text-[8px] sm:text-[10px] uppercase font-mono tracking-widest sm:tracking-[0.2em] mb-1.5 sm:mb-3 text-zinc-500 group-hover:text-cinematic-gold/80 transition-colors text-center w-full break-words">{t(config.language || 'vi', 'extract.standardExt')}</div>
            ...
            >
                ...
                <div className="text-[8px] sm:text-[10px] uppercase font-mono tracking-widest sm:tracking-[0.2em] mb-1.5 sm:mb-3 text-zinc-500 group-hover:text-cinematic-cyan/80 transition-colors text-center w-full break-words truncate">{t(config.language || 'vi', 'extract.quickExt')}</div>
                ...
                <div className="font-bold flex flex-col items-center justify-center gap-0.5 sm:gap-1.5 text-[8px] sm:text-sm font-mono w-full bg-black/40 p-1 sm:p-2 rounded-lg border border-white/5 group-hover:border-cinematic-cyan/20 mt-auto">
                   <div className="flex flex-col sm:flex-row items-center sm:justify-between w-full px-1 sm:px-2">
                       <span className="hidden sm:inline text-zinc-500 text-[10px] tracking-widest">{t(config.language || 'vi', 'extract.ticket')}</span>
                   ...
                   <div className="flex flex-col sm:flex-row items-center sm:justify-between w-full px-1 sm:px-2">
                       <span className="hidden sm:inline text-zinc-500 text-[10px] tracking-widest">{t(config.language || 'vi', 'extract.fee')}</span>
