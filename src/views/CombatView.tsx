...
export const CombatView: React.FC<Props> = ({
  ...
  const executeBattle = async () => {
    ...
    while (currentSquadHp > 0 && getTotalEnemyHp() > 0 && totalActions < MAX_ACTIONS) {
      ...
      if (activeIdx === -1) {
          ...
          for(let i=0; i<6; i++) {
             if (squad[i] && currentCardHps[i] > 0) minTicks = Math.min(minTicks, (1000 - squadATB[i]) / sqSpeeds[i]);
             if (enemySquad[i] && currentEnemyHps[i] > 0) minTicks = Math.min(minTicks, (1000 - enemyATB[i]) / enSpeeds[i]);
          }
          ...
          for (let step = 0; step < steps; step++) {
              for(let i=0; i<6; i++) {
                 if (squad[i] && currentCardHps[i] > 0) squadATB[i] += sqSpeeds[i] * tickPerStep;
                 if (enemySquad[i] && currentEnemyHps[i] > 0) enemyATB[i] += enSpeeds[i] * tickPerStep;
              }
      ...
      if (isSquadActor) {
        ...
        if (skipEnTurn) {
          addLog(
            <span><i className="fa-solid fa-ban text-red-500"></i> {activeEn.name} bị vô hiệu hóa! Bỏ qua lượt đánh.</span>,
  ...
  const renderEnemySlot = (bossData: Boss | null, index: number) => {
    ...
    const atb = typeof displayEnemyATB[index] === 'number' ? displayEnemyATB[index] : 0;
    ...
    return (
      <motion.div
        ...
        className={`w-24 h-36 lg:w-44 lg:h-60 rounded-xl flex flex-col relative group bg-black/40 border transition-all duration-300 shadow-lg ${isAttacking ? "border-red-500 shadow-[0_0_20px_rgba(239,68,68,0.4)]" : "border-white/10"}`}
      >
        ...
        <div className="absolute bottom-0 left-0 w-full p-1.5 flex flex-col gap-1">
          ...
          <div className="flex flex-col w-full gap-1">
            <div className="text-[8px] text-red-400 font-mono w-full">
              ...
              <div className="flex justify-between w-full mt-1 mb-[1px]">
                  <span className="text-[7px] text-yellow-500 font-bold"><i className="fa-solid fa-gauge-high"></i> ATB</span>
              </div>
              <div className="h-[2px] bg-white/10 w-full rounded-full overflow-hidden">
  ...
  const renderSquadSlot = (card: Card | null, index: number) => {
    ...
    return (
      <motion.div
        ...
        onClick={() => !inBattle && onOpenSquadSelector(index)}
        className={`w-24 h-36 lg:w-44 lg:h-60 rounded-xl flex flex-col relative group bg-black/40 border transition-all duration-300 shadow-lg ${isAttacking ? "border-cinematic-cyan shadow-[0_0_20px_rgba(0,243,255,0.4)]" : "border-white/10"}`}
      >
        <div className="absolute inset-0 rounded-xl overflow-hidden pointer-events-none">
          <img
            src={card.imageUrl}
            ...
            <div className="flex-1 flex flex-col gap-1">
              <div className="text-[8px] text-green-400 font-mono w-full">
                ...
                <div className="flex justify-between w-full mt-1 mb-[0.5px]">
                    <span className="text-[7px] text-blue-300 font-bold"><i className="fa-solid fa-bolt"></i> {Math.floor(displayCardManas[index] || 0)}/{displayCardMaxManas[index] || 100}</span>
                </div>
                <div className="h-[2px] bg-white/10 w-full rounded-full overflow-hidden">
                ...
                <div className="h-[2px] bg-white/10 w-full rounded-full overflow-hidden">
                    <div className="h-full bg-yellow-500 transition-all duration-300 shadow-[0_0_5px_rgba(234,179,8,0.5)]" style={{ width: `${Math.min(100, ((displaySquadATB[index] || 0) / 1000) * 100)}%` }}></div>
                </div>
              </div>
            </div>
          ...
          {activeCutInCard && (() => {
            const getCutInColors = (card: Card) => {
              switch (card.element) {
                ...
                default:
                  if (card.faction === "Light") return { bg: "bg-yellow-200/30", to: "to-yellow-200", stroke: "rgba(254,240,138,0.5)", border: "border-yellow-200/30" };
                  if (card.faction === "Dark") return { bg: "bg-purple-600/30", to: "to-purple-600", stroke: "rgba(147,51,234,0.5)", border: "border-purple-600/30" };
            ...
            {/* Control Area */}
             <div className="flex flex-col sm:flex-row justify-center items-center py-6 gap-6 relative">
              ...
              >
                ...
                <i className={`fa-solid fa-forward-fast text-xl ${combatSpeed > 1 ? 'text-yellow-400' : 'text-zinc-500'}`}></i>
                <div className="flex flex-col items-start gap-1">
