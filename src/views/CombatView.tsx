...
export const CombatView: React.FC<Props> = ({
  ...
  type WeatherType = 'clear' | 'acid_rain' | 'static_storm' | 'heatwave' | 'earthquake';
  const [activeWeather, setActiveWeather] = useState<WeatherType>('clear');

  const getWeatherInfo = (w: WeatherType) => {
    switch (w) {
      case 'clear': return { name: 'Trời Quang', desc: 'Sóng radar ổng định. Không có hiện tượng dị thường.', icon: 'fa-sun', color: 'text-zinc-400', bg: 'bg-zinc-900/50' };
      case 'acid_rain': return { name: 'Mưa Acid (Acid Rain)', desc: 'Hệ Thủy (Water) hồi 5% HP max mỗi lượt. Hệ Phong (Wind) bị trừ 20% Giáp (DEF).', icon: 'fa-cloud-rain', color: 'text-green-400', bg: 'bg-green-900/30' };
      case 'static_storm': return { name: 'Bão Tĩnh Điện (Static Storm)', desc: 'Hệ Lôi (Lightning) x2 Tốc độ Đánh (SPD).', icon: 'fa-bolt', color: 'text-yellow-400', bg: 'bg-yellow-900/30' };
      case 'heatwave': return { name: 'Nắng Gắt (Heatwave)', desc: 'Hệ Hỏa (Fire) +20% Sát thương Tấn công. Hệ Thủy (Water) -20% Tốc độ Đánh (SPD).', icon: 'fa-fire-alt', color: 'text-red-400', bg: 'bg-red-900/30' };
      case 'earthquake': return { name: 'Động Đất (Earthquake)', desc: 'Hệ Thổ (Earth) +30% Giáp (DEF).', icon: 'fa-mountain', color: 'text-orange-400', bg: 'bg-orange-900/30' };
      default: return { name: 'Trời Quang', desc: '', icon: 'fa-sun', color: 'text-zinc-400', bg: 'bg-zinc-900/50' };
    }
  };
  ...
  useEffect(() => {
    if (!inBattle) {
      const weathers: WeatherType[] = ['clear', 'acid_rain', 'static_storm', 'heatwave', 'earthquake'];
      setActiveWeather(weathers[Math.floor(Math.random() * weathers.length)]);
    }
  }, [opTab]);
      ...
      if (worldBossState.boss) {
        ...
        const gen = async () => {
          ...
          try {
            ...
            bossData.level = worldBossState.level;
            try {
              const dummyCard = { 
                gender: "Unknown", 
            ...
            setTimeout(() => {
               setLogs((prev: any[]) => [...prev, <div key={Math.random()} className="font-bold text-red-500 uppercase mb-1.5 animate-fade-in">{`CẢNH BÁO: PHÁT HIỆN WORLD BOSS CẤP ĐỘ ${worldBossState.level} TRONG ĐIỀU KIỆN KHÍ HẬU ${bossData.environment ? bossData.environment.toUpperCase() : "KHÔNG XÁC ĐỊNH"}!`}</div>].slice(-40));
            }, 500);
          } catch {
             // Let user retry
          } finally {
  ...
  useEffect(() => {
    if (!inBattle) {
      if (opTab === 'phantasm' && phantasmProgress) {
         let initialSquadHp = 0;
         ...
         setDisplaySquadHp(initialSquadHp);
         squadMaxHp.current = squadHp; // Keep max squad HP for percentage computations if needed, wait squadMaxHp is used for some things.
         setDisplayCardHps(initialCardHps);
      } else {
  ...
  }, [squadHp, inBattle, JSON.stringify(cardMaxHp), opTab, phantasmProgress]);
  ...
  const executeBattle = async () => {
    ...
    squad.forEach((card, i) => {
      ...
      if (activeWeather === 'heatwave' && card.element === 'Fire') {
          finalCardAtk = Math.floor(finalCardAtk * 1.2);
          addLog(
            <span>Nắng Gắt: {card.name} (Fire) <span className="text-red-400">+20% ATK</span></span>,
            "text-[9px] text-red-500 font-mono italic"
          );
      }
    ...
    squad.forEach((c, i) => {
        if (!c) return;
        let speed = Math.max(1, calculateCombatStats(c).speed || 100);
        
        if (activeWeather === 'static_storm' && c.element === 'Lightning') {
            speed *= 2;
            addLog(
                <span>Bão Tĩnh Điện: {c.name} (Lightning) <span className="text-yellow-400">x2 SPD</span></span>,
                "text-[9px] text-yellow-500 font-mono italic"
            );
        } else if (activeWeather === 'heatwave' && c.element === 'Water') {
            speed *= 0.8;
            addLog(
                <span>Nắng Gắt: {c.name} (Water) <span className="text-red-400">-20% SPD</span></span>,
                "text-[9px] text-red-500 font-mono italic"
            );
        }
    ...
    enemySquad.forEach((e, i) => { 
        if (!e) return;
        let speed = Math.max(1, e?.speed || getEnemySpeed(e));
        if (activeWeather === 'static_storm' && e.element === 'Lightning') speed *= 2;
        else if (activeWeather === 'heatwave' && e.element === 'Water') speed *= 0.8;
        enSpeeds[i] = speed;
    });
    ...
    while (currentSquadHp > 0 && getTotalEnemyHp() > 0 && totalActions < MAX_ACTIONS) {
      ...
      while (tacticalQueue.current.length > 0) {
        ...
        if (action === "strike") {
          ...
          playSkillSound();
          let globalHealAmt = 0;
          let globalShieldAmt = 0;
          currentCardHps = currentCardHps.map((hp, idx) => {
             if (squad[idx] && hp > 0) {
                if (squad[idx]!.element === 'Earth' || squad[idx]!.faction === 'Tech') {
                     const shieldAmt = Math.floor(cardMaxHp[idx] * 0.25);
                     squadShields[idx] += shieldAmt;
                     setDisplayShields([...squadShields]);
                     globalShieldAmt += shieldAmt;
                     return hp;
                } else {
          ...
          setDisplaySquadHp(currentSquadHp);
          addLog(
            <span>💉 <strong>CAN THIỆP CHIẾN THUẬT: EMERGENCY REPAIR!</strong> Khôi phục <span className="text-green-500 font-bold">+${globalHealAmt} HP</span> và <span className="text-zinc-300 font-bold">+${globalShieldAmt} Khiên</span>!</span>,
      ...
      if (activeWeather === 'acid_rain') {
         if (isSquadActor && squad[activeIdx]?.element === 'Water') {
             const h = Math.floor(targetMaxManas[activeIdx] * 0.05); // Just small heal based on missing HP? Wait no, max HP is cardMaxHp
             const healVal = Math.floor(cardMaxHp[activeIdx] * 0.05);
             if (currentCardHps[activeIdx] < cardMaxHp[activeIdx]) {
                 const diff = Math.min(healVal, cardMaxHp[activeIdx] - currentCardHps[activeIdx]);
                 if (diff > 0) {
                     currentCardHps[activeIdx] += diff;
                     addDamagePopup(diff, `ally-${activeIdx}` as any, false, undefined, "text-green-400", 0, true);
                     addLog(<span>Trời Mưa: <span className="text-green-400">+{diff} HP</span> cho {squad[activeIdx]?.name}</span>, "text-[9px] text-green-500 font-mono italic");
                 }
             }
         } else if (!isSquadActor && enemySquad[activeIdx]?.element === 'Water') {
             const healVal = Math.floor(enemySquad[activeIdx]!.hp * 0.05);
             currentEnemyHps[activeIdx] = Math.min(enemySquad[activeIdx]!.hp, currentEnemyHps[activeIdx] + healVal);
             addDamagePopup(healVal, `enemy-${activeIdx}` as any, false, undefined, "text-green-400", 0, true);
         }
      }
      ...
      if (isSquadActor) {
        ...
        if (role === "Vanguard") {
           ...
           let totalHealAmt = 0;
           let totalShieldAmt = 0;
           currentCardHps = currentCardHps.map((hp, i) => {
              if (squad[i] && hp > 0) {
                 if (squad[i]!.element === 'Earth' || squad[i]!.faction === 'Tech') {
                     // Add shield instead of heal for Earth/Tech
                     const shieldAmt = Math.floor(cardMaxHp[i] * 0.05);
                     squadShields[i] += shieldAmt;
                     setDisplayShields([...squadShields]);
                     totalShieldAmt += shieldAmt;
                     return hp;
                 } else {
                     addDamagePopup(healAmt, `squad_${i}`, false, undefined, "text-green-400", 0, true);
                     totalHealAmt += healAmt;
        ...
        for (const targetIdx of targetEnIdxs) {
           ...
           let enemyImplicitDef = targetEnemy.attack * 0.4;
           if (activeWeather === 'acid_rain' && targetEnemy.element === 'Wind') {
               enemyImplicitDef = Math.floor(enemyImplicitDef * 0.8);
           } else if (activeWeather === 'earthquake' && targetEnemy.element === 'Earth') {
               enemyImplicitDef = Math.floor(enemyImplicitDef * 1.3);
           }
           const reductionRate = enemyImplicitDef / (enemyImplicitDef + 1000);
           currentAtk = Math.floor(currentAtk * (0.9 + Math.random() * 0.2));
           
           let pierceBypass = 0;
           if (isUltimate && attackerCard.element === 'Lightning') {
               pierceBypass = 1.0;
               synergyLog = <>{synergyLog}<span className="ml-2 text-[9px] px-1.5 py-0.5 rounded font-mono border border-cyan-500/50 bg-cyan-900/40 text-cyan-400 uppercase">⚡ Bỏ Qua Giáp</span></>;
           } else if (attackerCard.genes?.includes('gene_armor_pierce')) {
               pierceBypass = 0.2;
               synergyLog = <>{synergyLog}<span className="ml-2 text-[9px] px-1.5 py-0.5 rounded font-mono border border-purple-500/50 bg-purple-900/40 text-purple-400 uppercase">Phá Giáp 20%</span></>;
           }

           currentAtk = Math.max(1, Math.floor(currentAtk * (1 - (reductionRate * (1 - pierceBypass)))));
        ...
        if (skipEnTurn) {
          ...
          let reductionRate = Math.min(0.8, squadDefenseValue / (squadDefenseValue + 1000));
          ...
          const aliveVanguards = allAlive.filter(i => squad[i] && getCardRole(squad[i]!) === "Vanguard");
          ...
          if (enemyBehavior === "tactical") {
             ...
             if (aliveVanguards.length > 0) targetIdx = aliveVanguards[Math.floor(Math.random() * aliveVanguards.length)];
             else if (aliveFrontline.length > 0) targetIdx = aliveFrontline[Math.floor(Math.random() * aliveFrontline.length)];
             ...
             if (bossAttackIsPhysical) {
                ...
                if (aliveVanguards.length > 0 && Math.random() < 0.6) targetIdx = aliveVanguards[Math.floor(Math.random() * aliveVanguards.length)];
                else if (allAlive.length > 0) targetIdx = allAlive[Math.floor(Math.random() * allAlive.length)];
             }
          }
          ...
          if (actionType !== "heal" && targetIdx !== -1 && squad[targetIdx]) {
              const targetCard = squad[targetIdx]!;
              const targetStats = calculateCombatStats(targetCard);
              let baseDef = bossAttackIsPhysical ? targetStats.def : targetStats.res;
              
              if (activeWeather === 'acid_rain' && targetCard.element === 'Wind') {
                  baseDef = Math.floor(baseDef * 0.8);
              } else if (activeWeather === 'earthquake' && targetCard.element === 'Earth') {
                  baseDef = Math.floor(baseDef * 1.3);
              }
              
              // Only override if the target has defensive modifiers that differ from global,
              // but since we want weather to have impact, we calculate specifically for them,
              // or just apply weather multiplier to the global reduction rate!
              // For simplicity, we apply a direct multiplier to the reductionRate based on weather.
              if (activeWeather === 'acid_rain' && targetCard.element === 'Wind') {
                  reductionRate *= 0.8;
              } else if (activeWeather === 'earthquake' && targetCard.element === 'Earth') {
                  reductionRate = Math.min(0.8, reductionRate * 1.3);
              }
          }
          ...
          if (Math.random() * 100 < dodgeRate) {
            ...
            if (actionType === "heal" && targetIdx !== -1) {
                ...
                let reflectDmg = 0;
                let reflectLog = null;
                // If Earth/Tech Vanguard, reflect damage
                if (targetIdx !== -1 && squad[targetIdx]) {
                     const tCard = squad[targetIdx]!;
                     if ((tCard.element === "Earth" || tCard.faction === "Tech") && getCardRole(tCard) === "Vanguard") {
                          reflectDmg = Math.floor(calculateCombatStats(tCard).def * 0.5);
                          if (reflectDmg > 0) {
                              reflectLog = <span className="ml-2 text-[9px] px-1.5 py-0.5 rounded font-mono border border-orange-500/50 bg-orange-900/40 text-orange-400 uppercase">🛡️ Phản Xạ {reflectDmg} DMG</span>;
                          }
                     }
                }
                ...
                if (targetIdx !== -1) {
                    ...
                    if (reflectDmg > 0) {
                        ...
                        if (currentEnemyHps[attackerEnIdx] <= 0) {
                            addLog(<span className="text-red-400">💀 Kẻ địch tử vong do phản sát thương!</span>, "text-red-400 text-xs bg-red-900/30 px-2 py-1 rounded font-serif border border-red-500/30");
                        }
                    }
  ...
  return (
    <div className="w-full flex flex-col items-center animate-fade-in pb-12">
      ...
      <div className="w-full max-w-6xl bg-zinc-950/80 border border-cinematic-cyan/20 ring-1 ring-cinematic-cyan/10 backdrop-blur-xl rounded-3xl p-4 sm:p-8 mb-8 relative overflow-hidden shadow-[inset_0_0_100px_rgba(0,0,0,0.8),0_0_40px_rgba(0,243,255,0.05)]">
        ...
        {/* WEATHER DISPLAY */}
        {(opTab === 'single_boss' || opTab === 'battlefield') && !inBattle && (
          <div className="flex justify-center mb-8 relative z-10 w-full max-w-2xl mx-auto">
             <div className={`p-4 rounded-xl border w-full flex items-center justify-between shadow-lg backdrop-blur-md ${getWeatherInfo(activeWeather).bg} border-${getWeatherInfo(activeWeather).color.replace('text-', '')}/30`}>
                <div className="flex items-center gap-4">
                   <div className={`w-12 h-12 rounded-lg bg-black/40 flex items-center justify-center shrink-0 border border-white/10 ${getWeatherInfo(activeWeather).color}`}>
                      <Icon name={`${getWeatherInfo(activeWeather).icon} text-2xl`} />
                   </div>
                   <div>
                       <h3 className={`font-mono text-sm font-bold uppercase tracking-widest mb-1 ${getWeatherInfo(activeWeather).color}`}>{getWeatherInfo(activeWeather).name}</h3>
                       <p className="text-[10px] sm:text-xs text-zinc-300 font-serif leading-relaxed">{getWeatherInfo(activeWeather).desc}</p>
                   </div>
                </div>
             </div>
          </div>
                  ...
                  {!isLiteMode && damagePopups
                    ...
                    .map((p) => (
                      <motion.div
                        ...
                        className={`absolute left-1/2 top-1/4 -translate-x-1/2 flex flex-col items-center justify-center pointer-events-none z-[200] font-black ${p.colorClass || "text-red-500"} text-6xl sm:text-7xl whitespace-nowrap font-serif italic tracking-tighter ${isMobileEnv ? "drop-shadow-md" : "drop-shadow-[0_0_15px_rgba(220,38,38,0.8)]"}`}
            ...
            >
              ...
              <button 
                onClick={() => {
                  ...
                  if (onCombatReturn) onCombatReturn();
                }}
