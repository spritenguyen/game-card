...
export const AscensionView: React.FC<Props> = ({ config, currency, modifyCurrency, inventory, cards, modifyInventory, onCompleteAscension, onError, onAlert, isGlobalProcessing, setGlobalProcessing }) => {
    ...
    const renderCardSlot = (c: Card, clearSlot: () => void, isCore: boolean) => (
        <div className={`w-full h-full relative border ${isCore ? 'border-cinematic-gold/80 shadow-[0_0_20px_rgba(255,184,0,0.3)]' : 'border-red-500/80 shadow-[0_0_20px_rgba(239,68,68,0.3)]'} rounded-xl overflow-hidden group`}>
            ...
            {isCore ? (
                <div className="absolute top-1 left-1 sm:top-2 sm:left-2 bg-cinematic-gold text-black px-1 sm:px-2 py-0.5 rounded text-[6px] sm:text-[8px] font-bold uppercase tracking-widest"><Icon name="fa-crown sm:mr-1" className="fa-crown sm:mr-1" /><span className="hidden sm:inline">Vật Thể Gốc</span></div>
            ) : (
                <div className="absolute top-1 left-1 sm:top-2 sm:left-2 bg-red-600 text-white px-1 sm:px-2 py-0.5 rounded text-[6px] sm:text-[8px] font-bold uppercase tracking-widest"><Icon name="fa-fire sm:mr-1" className="fa-fire sm:mr-1" /><span className="hidden sm:inline">Tế Vật</span></div>
            )}
            <button 
    ...
    return (
        <div className="w-full flex flex-col items-center animate-fade-in pb-12">
            <div className="w-full max-w-5xl bg-cinematic-900/40 border border-white/5 ring-1 ring-white/5 backdrop-blur-md rounded-3xl p-6 sm:p-10 mb-8 relative overflow-hidden shadow-[inset_0_0_80px_rgba(0,0,0,0.5),0_0_40px_rgba(255,184,0,0.05)]">
                ...
                <h2 className="text-center font-serif text-3xl text-cinematic-gold mb-2 tracking-widest relative z-10"><Icon name="fa-hammer mr-3" className="fa-hammer mr-3" />LÒ RÈN KHỞI NGUYÊN (UR)</h2>
                <div className="text-center mb-10 relative z-10">
                    ...
                    <p className="text-[9px] text-red-400 uppercase tracking-widest bg-red-900/20 py-1 inline-block px-4 border border-red-900/50 rounded-full mx-auto mt-2"><Icon name="fa-triangle-exclamation mr-1" className="fa-triangle-exclamation mr-1" /> Thẻ làm Tế Vật & Vật Thể Gốc sẽ biến mất</p>
                </div>
                ...
                <div className="flex flex-col items-center relative z-10 w-full mb-10">
                    <h3 className="text-[12px] text-white/70 font-mono tracking-widest uppercase mb-4 text-center">
                        Thiết Lập Định Hướng Thức Tỉnh <br/>
                        <span className="text-[9px] text-white/40 block mt-1">Ảnh hưởng đến chỉ số kỹ năng tối thượng</span>
                    </h3>
                    <div className="flex flex-wrap justify-center gap-2 sm:gap-4 max-w-2xl px-2">
                        {[
                            { id: 'balanced', label: 'Cân Bằng', icon: 'fa-scale-balanced', color: 'text-zinc-300', bg: 'bg-zinc-800/80', ring: 'ring-zinc-400/50' },
                            { id: 'destruction', label: 'Hủy Diệt', icon: 'fa-meteor', color: 'text-red-400', bg: 'bg-red-900/40', ring: 'ring-red-500/50' },
                            { id: 'aegis', label: 'Hộ Mệnh', icon: 'fa-shield-halved', color: 'text-blue-400', bg: 'bg-blue-900/40', ring: 'ring-blue-500/50' },
                            { id: 'velocity', label: 'Siêu Tốc', icon: 'fa-wind', color: 'text-green-400', bg: 'bg-green-900/40', ring: 'ring-green-500/50' },
                            { id: 'enigma', label: 'Vô Cực', icon: 'fa-infinity', color: 'text-purple-400', bg: 'bg-purple-900/40', ring: 'ring-purple-500/50' }
                        ].map(path => (
                            <button
                                key={path.id}
                ...
                <div className="flex flex-col items-center relative z-10 w-full mb-6 py-4 bg-black/40 rounded-2xl border border-white/5">
                    <div className="text-[10px] text-cinematic-muted uppercase tracking-widest mb-3 flex items-center gap-2">
                        <Icon name="fa-flask" className="fa-flask" /> Vật phẩm Xúc Tác Cần Thiết
                    </div>
                        ...
                        <div className="flex-1 overflow-y-auto no-scrollbar grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 content-start">
                            ...
                            {getAvailableCardsForSlot().length === 0 && (
                                <div className="col-span-full py-12 text-center border-2 border-dashed border-white/10 rounded-xl">
                                    <p className="text-white/40 uppercase tracking-widest text-[10px]"><Icon name="fa-box-open mr-2 text-lg mb-2" className="fa-box-open mr-2 text-lg mb-2" /><br/>Chưa có thẻ SSR nào khả dụng trong kho chứa.</p>
                                </div>
                            )}
                        </div>
