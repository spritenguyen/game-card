...
export const AscensionView: React.FC<Props> = ({ config, currency, modifyCurrency, inventory, cards, modifyInventory, onCompleteAscension, onError, onAlert, isGlobalProcessing, setGlobalProcessing }) => {
    ...
    const [ascensionPath, setAscensionPath] = useState<'destruction' | 'aegis' | 'velocity' | 'enigma' | 'balanced'>('balanced');
    ...
    const handleSelectCard = (id: string) => {
        ...
        if (selectorTarget === 'core') {
            if (c.cardClass !== 'SSR') return onError(t('ascension.errorCore'));
            if (sac1Slot?.id === id || sac2Slot?.id === id) return onError(t('ascension.errorUsed'));
            setCoreSlot(c);
        } else if (selectorTarget === 'sac1') {
            if (c.cardClass !== 'SSR') return onError(t('ascension.errorSacrifice'));
            if (coreSlot?.id === id || sac2Slot?.id === id) return onError(t('ascension.errorCoreUsed'));
            setSac1Slot(c);
        } else if (selectorTarget === 'sac2') {
            if (c.cardClass !== 'SSR') return onError(t('ascension.errorSacrifice'));
            if (coreSlot?.id === id || sac1Slot?.id === id) return onError(t('ascension.errorCoreUsed'));
            setSac2Slot(c);
        }
    ...
    const executeAscension = async () => {
        ...
        if (currency < DC_COST) return onError(t('ascension.errorCredits', { dc: DC_COST }));
        if ((inventory.quantumDust || 0) < DUST_COST) return onError(t('ascension.errorDust', { dust: DUST_COST }));

        setGlobalProcessing(true);
        try {
            if (currency < DC_COST) {
                setGlobalProcessing(false);
                return onError(t('ascension.declined'));
            }
            modifyCurrency(-DC_COST);
            modifyInventory(0, 0, {}, -DUST_COST);

            const cardData = await generateAscensionFromAI(coreSlot, ascensionPath, config);
            ...
            onAlert(t('ascension.successTitle'), t('ascension.successMsg'));
            
        } catch(e: any) {
            modifyCurrency(DC_COST);
            modifyInventory(0, 0, {}, DUST_COST); // Refund dust
            if(e.message === "API_KEY_INVALID") onAlert(t('ascension.systemName'), t('ascension.keyInvalid'));
            else onError(t('ascension.generalError'));
        } finally {
            setGlobalProcessing(false);
        }
    };
    ...
    const renderEmptySlot = (label: string, isCore: boolean) => (
        <div className={`w-full h-full flex flex-col items-center justify-center border-2 border-dashed ${isCore ? 'border-cinematic-gold/50 bg-black/60 shadow-[inset_0_0_50px_rgba(255,184,0,0.1)]' : 'border-red-500/50 bg-black/40 shadow-[inset_0_0_30px_rgba(239,68,68,0.1)]'} rounded-xl hover:bg-white/5 transition-all text-center p-1 sm:p-2 cursor-pointer relative overflow-hidden group`}> 
            ...
            <span className="text-[7px] sm:text-[9px] text-white/40 mt-1.5 uppercase font-mono border border-white/10 px-2 py-0.5 rounded-sm bg-black/50 z-10 block">{t('ascension.reqSSR')}</span>
        </div>
    );
    ...
    const renderCardSlot = (c: Card, clearSlot: () => void, isCore: boolean) => (
        <div className={`w-full h-full relative border ${isCore ? 'border-cinematic-gold/80 shadow-[0_0_20px_rgba(255,184,0,0.3)]' : 'border-red-500/80 shadow-[0_0_20px_rgba(239,68,68,0.3)]'} rounded-xl overflow-hidden group`}>
            ...
            {isCore ? (
                <div className="absolute top-1 left-1 sm:top-2 sm:left-2 bg-cinematic-gold text-black px-1 sm:px-2 py-0.5 rounded text-[6px] sm:text-[8px] font-bold uppercase tracking-widest"><Icon name="fa-crown sm:mr-1" className="fa-crown sm:mr-1" /><span className="hidden sm:inline">{t('ascension.core')}</span></div>
            ) : (
                <div className="absolute top-1 left-1 sm:top-2 sm:left-2 bg-red-600 text-white px-1 sm:px-2 py-0.5 rounded text-[6px] sm:text-[8px] font-bold uppercase tracking-widest"><Icon name="fa-fire sm:mr-1" className="fa-fire sm:mr-1" /><span className="hidden sm:inline">{t('ascension.targetSacrifice')}</span></div>
            )}
            <button 
            ...
            >
                <Icon name="fa-xmark text-[8px] sm:text-[10px]" className="fa-xmark text-[8px] sm:text-[10px]" />
            </button>
        </div>
    );
    ...
    return (
        <div className="w-full flex flex-col items-center animate-fade-in pb-12">
            <div className="w-full max-w-5xl bg-cinematic-900/40 border border-white/5 ring-1 ring-white/5 backdrop-blur-md rounded-3xl p-6 sm:p-10 mb-8 relative overflow-hidden shadow-[inset_0_0_80px_rgba(0,0,0,0.5),0_0_40px_rgba(255,184,0,0.05)]">
                ...
                <h2 className="text-center font-serif text-3xl text-cinematic-gold mb-2 tracking-widest relative z-10"><Icon name="fa-hammer mr-3" className="fa-hammer mr-3" />{t('ascension.title')}</h2>
                <div className="text-center mb-10 relative z-10">
                    <p className="text-[10px] text-cinematic-gold/70 tracking-widest uppercase block mt-1">{t('ascension.subtitle')}</p>
                    <p className="text-[9px] text-red-400 uppercase tracking-widest bg-red-900/20 py-1 inline-block px-4 border border-red-900/50 rounded-full mx-auto mt-2"><Icon name="fa-triangle-exclamation mr-1" className="fa-triangle-exclamation mr-1" /> {t('ascension.warning')}</p>
                </div>
                ...
                <div className="flex flex-row justify-center items-end gap-3 sm:gap-8 relative z-10 w-full mb-10">
                    ...
                    >
                        {sac1Slot ? renderCardSlot(sac1Slot, () => setSac1Slot(null), false) : renderEmptySlot(t('ascension.sacrifice1'), false)}
                    </div>
                    ...
                    >
                        ...
                        {coreSlot ? renderCardSlot(coreSlot, () => setCoreSlot(null), true) : renderEmptySlot(t('ascension.core'), true)}
                        
                        {/* Energy beams if both sacs exist */}
                        {sac1Slot && coreSlot && sac2Slot && (
                            <div className="absolute inset-0 pointer-events-none -z-10">
                                <div className="absolute top-1/2 -left-20 w-20 h-1 bg-cinematic-cyan/50 blur-[2px] animate-pulse origin-right shadow-[0_0_15px_rgba(0,243,255,0.8)]"></div>
                                <div className="absolute top-1/2 -right-20 w-20 h-1 bg-cinematic-cyan/50 blur-[2px] animate-pulse origin-left shadow-[0_0_15px_rgba(0,243,255,0.8)]"></div>
                            </div>
                        )}
                    </div>
                    ...
                    <div 
                        ...
                        className="w-24 h-36 sm:w-48 sm:h-72 cursor-pointer transform hover:-translate-y-2 transition-transform shrink-0"
                    >
                        {sac2Slot ? renderCardSlot(sac2Slot, () => setSac2Slot(null), false) : renderEmptySlot(t('ascension.sacrifice2'), false)}
                    </div>
                </div>

                {/* Path Selector */}
                <div className="flex flex-col items-center relative z-10 w-full mb-10">
                    <h3 className="text-[12px] text-white/70 font-mono tracking-widest uppercase mb-4 text-center">
                        {t('ascension.pathSetup')} <br/>
                        <span className="text-[9px] text-white/40 block mt-1">{t('ascension.pathEffect')}</span>
                    </h3>
                    <div className="flex flex-wrap justify-center gap-2 sm:gap-4 max-w-2xl px-2">
                        {[
                            { id: 'balanced', label: t('ascension.paths.balanced'), icon: 'fa-scale-balanced', color: 'text-zinc-300', bg: 'bg-zinc-800/80', ring: 'ring-zinc-400/50' },
                            { id: 'destruction', label: t('ascension.paths.destruction'), icon: 'fa-meteor', color: 'text-red-400', bg: 'bg-red-900/40', ring: 'ring-red-500/50' },
                            { id: 'aegis', label: t('ascension.paths.aegis'), icon: 'fa-shield-halved', color: 'text-blue-400', bg: 'bg-blue-900/40', ring: 'ring-blue-500/50' },
                            { id: 'velocity', label: t('ascension.paths.velocity'), icon: 'fa-wind', color: 'text-green-400', bg: 'bg-green-900/40', ring: 'ring-green-500/50' },
                            { id: 'enigma', label: t('ascension.paths.enigma'), icon: 'fa-infinity', color: 'text-purple-400', bg: 'bg-purple-900/40', ring: 'ring-purple-500/50' }
                        ].map(path => (
                            <button
                                key={path.id}
                                onClick={() => !isGlobalProcessing && setAscensionPath(path.id as any)}
                                className={`px-4 sm:px-6 py-3 rounded-full border transition-all duration-300 flex flex-col items-center gap-1 ${
                                    ascensionPath === path.id 
                                        ? `border-transparent ring-2 ${path.ring} ${path.bg} shadow-[0_0_20px_var(--tw-ring-color)] scale-105` 
                                        : 'border-white/10 hover:border-white/30 bg-black/40 text-white/50 hover:text-white/80'
                                }`}
                            >
                                <Icon name={`${path.icon} ${ascensionPath === path.id ? path.color : ''} text-lg sm:text-xl`} className={`${path.icon} ${ascensionPath === path.id ? path.color : ''} text-lg sm:text-xl`} />
                ...
                <div className="flex flex-col items-center relative z-10 w-full mb-6 py-4 bg-black/40 rounded-2xl border border-white/5">
                    <div className="text-[10px] text-cinematic-muted uppercase tracking-widest mb-3 flex items-center gap-2">
                        <Icon name="fa-flask" className="fa-flask" /> {t('ascension.catalysts')}
                    </div>
                ...
                <div className="flex justify-center relative z-10">
                    ...
                    >   
                        <Icon  className={`${isGlobalProcessing ? 'fa-circle-notch fa-spin' : 'fa-bolt-lightning'} text-xl`} />
                        <span className="text-sm">
                            {isGlobalProcessing ? t('ascension.processing') : (canAscend ? t('ascension.button') : t('ascension.insufficient'))}
                        </span>
                    </button>
                </div>
            </div>
                    ...
                    <div className="relative z-10 w-full max-w-4xl max-h-[80vh] flex flex-col bg-cinematic-900 border border-white/10 rounded-2xl p-6 shadow-2xl">
                        <div className="flex justify-between items-center mb-6 shrink-0">
                            <h3 className="text-xl font-serif text-white tracking-widest uppercase">
                                {t('ascension.selectTarget')} <span className={selectorTarget === 'core' ? 'text-cinematic-gold' : 'text-red-400'}>{selectorTarget === 'core' ? t('ascension.targetCore') : t('ascension.targetSacrifice')}</span>
                            </h3>
                            <button onClick={() => setSelectorTarget(null)} className="text-white/50 hover:text-white transition-colors">
                                <Icon name="fa-xmark text-2xl" className="fa-xmark text-2xl" />
                            </button>
                        </div>
                        <div className="flex-1 overflow-y-auto no-scrollbar grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 content-start">
                            ...
                            {getAvailableCardsForSlot().length === 0 && (
                                <div className="col-span-full py-12 text-center border-2 border-dashed border-white/10 rounded-xl">
                                    <p className="text-white/40 uppercase tracking-widest text-[10px]"><Icon name="fa-box-open mr-2 text-lg mb-2" className="fa-box-open mr-2 text-lg mb-2" /><br/>{t('ascension.noSSR')}</p>
                                </div>
                            )}
                        </div>
                        <div className="shrink-0 mt-4 pt-4 border-t border-white/10">
                            <button onClick={() => setSelectorTarget(null)} className="w-full py-3 bg-white/5 hover:bg-white/10 rounded-xl text-white font-bold tracking-widest uppercase text-[10px]">{t('ascension.cancel')}</button>
                        </div>
                    </div>
                </div>,
                document.body
            ...
            <Dialog
                isOpen={showConfirm}
                title={t('ascension.dialogTitle')}
                message={t('ascension.dialogMsg', { dc: DC_COST, dust: DUST_COST })}
                type="confirm"
