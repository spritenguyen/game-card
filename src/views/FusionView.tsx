...
export const FusionView: React.FC<Props> = ({ config, currency, modifyCurrency, inventory, cards, modifyInventory, fusionSlot1, fusionSlot2, setFusionSlot1, setFusionSlot2, onOpenSelector, onCompleteFusion, onError, onAlert, isGlobalProcessing, setGlobalProcessing }) => {
    ...
    const executeFusion = async () => {
        ...
        try {
            ...
            let newFaction = fusionSlot1.faction || 'CyberCore';
            ...
            if (upgradeItem) {
                if (upgradeItem.type === 'item' && upgradeItem.id !== 'quantumDust') {
                    ...
                    // Just take a trait from the sacrificed card as a guaranteed override
                    // No random mutation chance anymore
                    isMutated = true;
                    if (Math.random() < 0.5) forcedFaction = upgradeItem.card.faction;
                    else forcedElement = upgradeItem.card.element;
                }
            }
            ...
            if (forcedFaction || forcedElement) { 
                isMutated = true;
                if (forcedFaction) {
            ...
            if (isMutated) {
                ...
                // TODO: translate inside the code
    ...
    const renderSlot = (c: Card | null, label: string, desc: string, slotNum: 1 | 2) => {
        if (c) {
            return (
                <div className="w-full h-full p-2 flex flex-col">
                    ...
                    >
                        <Icon name="fa-xmark text-xs sm:text-sm" className="fa-xmark text-xs sm:text-sm" />
                    </button>
                </div>
            );
        }
        return (
            <>
                <Icon name="fa-plus text-3xl sm:text-4xl text-white/20 mb-3 group-hover:text-white/40 transition-colors" className="fa-plus text-3xl sm:text-4xl text-white/20 mb-3 group-hover:text-white/40 transition-colors" />
    ...
    return (
        <div className="w-full flex flex-col items-center animate-fade-in pb-12">
            <div className="w-full max-w-4xl bg-cinematic-900/40 border border-white/5 ring-1 ring-white/5 backdrop-blur-md rounded-3xl p-6 sm:p-10 mb-8 relative overflow-hidden shadow-[inset_0_0_80px_rgba(0,0,0,0.5),0_0_40px_rgba(0,243,255,0.05)]">
                ...
                <div className="flex flex-wrap items-center justify-center gap-2 mb-6 relative z-10">
                    <div className="bg-black/60 border border-cinematic-gold/30 px-3 py-1 rounded-full text-[10px] sm:text-xs flex items-center gap-2 shadow-lg">
                        <Icon name="fa-meteor text-cinematic-gold" className="fa-meteor text-cinematic-gold" />
                    ...
                    {Object.entries(inventory.materials || {})
                        ...
                        .map(([name, count]) => (
                            <div key={name} className="bg-black/60 border border-cinematic-cyan/30 px-3 py-1 rounded-full text-[10px] sm:text-xs flex items-center gap-2 shadow-lg">
                                <Icon name="fa-gem text-cinematic-cyan" className="fa-gem text-cinematic-cyan" />
                ...
                <h2 className="text-center font-serif text-2xl text-white mb-2 tracking-widest relative z-10">{t(config.language || 'vi', 'forge.chimeraTitle')}</h2>
                <div className="text-center mb-10 relative z-10">
                    <p className="text-[10px] text-red-400 uppercase tracking-widest bg-red-900/20 py-1 inline-block px-4 border border-red-900/50 rounded-full mx-auto mb-2"><Icon name="fa-triangle-exclamation mr-2" className="fa-triangle-exclamation mr-2" /> {t(config.language || 'vi', 'forge.chimeraWarning1')}</p>
                    <p className="text-[9px] text-cinematic-cyan/70 tracking-widest uppercase block mt-1">{t(config.language || 'vi', 'forge.chimeraWarning2')}</p>
                </div>
                ...
                <div className="flex flex-col items-center relative z-10">
                    ...
                    <div className="flex flex-row items-center justify-center gap-4 sm:gap-12 w-full mb-8">
                        <div onClick={() => onOpenSelector(1)} className={`w-36 h-56 sm:w-56 sm:h-80 fusion-slot rounded-2xl flex flex-col items-center justify-center cursor-pointer relative group overflow-hidden bg-black/40 backdrop-blur-sm shadow-[0_0_15px_rgba(0,0,0,0.5)] border border-white/5 hover:border-cinematic-cyan/30 transition-all ${fusionSlot1 ? 'filled border-cinematic-cyan/50' : ''}`}>
                            {renderSlot(fusionSlot1, t(config.language || 'vi', 'forge.alphaSlot'), t(config.language || 'vi', 'forge.alphaDesc'), 1)}
                        </div>

                        <div className="flex flex-col items-center justify-center relative hidden sm:flex">
                           <Icon name="fa-link text-2xl text-cinematic-cyan/30" className="fa-link text-2xl text-cinematic-cyan/30" />
                        </div>
                        ...
                        <div onClick={() => onOpenSelector(2)} className={`w-36 h-56 sm:w-56 sm:h-80 fusion-slot rounded-2xl flex flex-col items-center justify-center cursor-pointer relative group overflow-hidden bg-black/40 backdrop-blur-sm shadow-[0_0_15px_rgba(0,0,0,0.5)] border border-white/5 hover:border-cinematic-cyan/30 transition-all ${fusionSlot2 ? 'filled border-cinematic-cyan/50' : ''}`}>
                            {renderSlot(fusionSlot2, t(config.language || 'vi', 'forge.omegaSlot'), t(config.language || 'vi', 'forge.omegaDesc'), 2)}
                        </div>
                    </div>
                    ...
                    <div className="relative flex flex-col items-center mb-10">
                        ...
                        >
                            {upgradeItem ? (
                                upgradeItem.type === 'item' ? (
                                    <>
                                        <Icon  className={`${upgradeItem.id === 'quantumDust' ? 'fa-meteor' : 'fa-gem'} text-xl sm:text-3xl text-cinematic-gold mb-1`} />
                                        ...
                                        ><Icon name="fa-xmark" className="fa-xmark" /></button>
                                    </>
                                ) : (
                                    <>
                                        ...
                                        <Icon name="fa-skull text-xl sm:text-2xl text-red-500 mb-1 z-10 drop-shadow-[0_0_5px_rgba(0,0,0,0.8)]" className="fa-skull text-xl sm:text-2xl text-red-500 mb-1 z-10 drop-shadow-[0_0_5px_rgba(0,0,0,0.8)]" />
                                ...
                                <>
                                    <Icon name="fa-plus text-xl sm:text-2xl text-cinematic-gold/40 group-hover:text-cinematic-gold/80 transition-colors mb-1" className="fa-plus text-xl sm:text-2xl text-cinematic-gold/40 group-hover:text-cinematic-gold/80 transition-colors mb-1" />
                                    <span className="text-[6.5px] sm:text-[8px] uppercase tracking-widest text-center whitespace-nowrap leading-none mt-1 shadow-black drop-shadow-md">
                                        <span className="text-cinematic-gold/50 group-hover:text-cinematic-gold/80">{t(config.language || 'vi', 'forge.catalyst')}</span>
                                        <span className="text-zinc-600 mx-1">|</span>
                                        <span className="text-red-500/80 group-hover:text-red-400">{t(config.language || 'vi', 'forge.canSacrifice')}</span>
                                    </span>
                                </>
                            )}
                        </div>
                        {upgradeItem ? (
                            <div className="absolute -bottom-6 w-max text-[8px] sm:text-[9px] text-cinematic-gold bg-cinematic-gold/10 px-3 py-1 rounded-full border border-cinematic-gold/30 shadow-lg whitespace-nowrap tracking-wider z-10 flex gap-2">
                                ...
                                {upgradeItem.type === 'item' && upgradeItem.id !== 'quantumDust' && <span className="text-cinematic-cyan">{upgradeItem.id.endsWith(' Core') ? `Lai Tộc ${upgradeItem.id.replace(' Core', '')}` : upgradeItem.id.endsWith(' Shard') ? `Hệ ${upgradeItem.id.replace(' Shard', '')}` : `Can thiệp cấu trúc gen`}</span>}
                            </div>
                        ) : (
                            <div className="absolute -bottom-6 w-max text-[8px] sm:text-[9px] text-cinematic-muted tracking-widest uppercase">
                                {t(config.language || 'vi', 'forge.optional')}
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex flex-col items-center w-full max-w-sm relative">
                        <div className="bg-cinematic-900 border border-cinematic-cyan/30 text-cinematic-cyan text-[10px] sm:text-xs px-4 py-1.5 rounded-full font-bold shadow-lg mb-4 whitespace-nowrap tracking-widest">
                            {t(config.language || 'vi', 'forge.fee')} {canFuse ? `${dynamicCost} DC` : t(config.language || 'vi', 'forge.feeFormula')}
                        </div>
                        
                        {canFuse && maxR >= 1 && (
                            <div className="text-[9px] text-cinematic-muted uppercase tracking-widest mb-4 flex flex-col items-center gap-1">
                                <span className="flex items-center gap-2"><Icon name="fa-circle-info" className="fa-circle-info" /> {t(config.language || 'vi', 'forge.autoDeduct')}</span>
                        ...
                        {canFuse && (
                            <div className="flex flex-col items-center mb-6 w-full px-2">
                                <div className="text-[10px] text-cinematic-muted uppercase tracking-widest mb-2 flex items-center gap-2">
                                    <Icon name="fa-chart-pie" className="fa-chart-pie" /> {t(config.language || 'vi', 'forge.expectedRate')}
                                </div>
                                <div className="flex flex-wrap justify-center gap-2 w-full">
                                    ...
                                    <div className="flex flex-col items-center bg-cinematic-cyan/10 border border-cinematic-cyan/30 rounded-lg px-3 py-1 min-w-[60px] max-w-[80px]">
                                        <span className="text-xs font-bold text-cinematic-cyan text-center truncate w-full px-1">{upgradeItem && upgradeItem.type === 'item' && upgradeItem.id.endsWith(' Core') ? `Tộc ${upgradeItem.id.replace(' Core', '')}` : upgradeItem && upgradeItem.type === 'item' && upgradeItem.id.endsWith(' Shard') ? `Hệ ${upgradeItem.id.replace(' Shard', '')}` : 'Lai Cấu Trúc'}</span>
                                        <span className="text-[10px] text-white/80 uppercase">100%</span>
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        >
                            <Icon  className={`${isGlobalProcessing ? 'fa-circle-notch fa-spin' : 'fa-dna'} text-xl sm:text-2xl`} />
                            <span className="text-sm sm:text-base">{isGlobalProcessing ? t(config.language || 'vi', 'forge.btnProcessing') : (canFuse ? t(config.language || 'vi', 'forge.btnStart') : t(config.language || 'vi', 'forge.btnWait'))}</span>
                        </button>
                    </div>
                </div>
            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative z-10 w-full max-w-md bg-cinematic-900 border border-cinematic-gold/30 rounded-2xl p-6 shadow-[0_0_50px_rgba(255,184,0,0.1)] flex flex-col max-h-[90vh]">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-lg font-serif text-cinematic-gold tracking-widest uppercase"><Icon name="fa-flask mr-2" className="fa-flask mr-2" /> {t(config.language || 'vi', 'forge.catalystModal.title')}</h3>
                            <button onClick={() => setShowUpgradeModal(false)} className="text-white/50 hover:text-white"><Icon name="fa-xmark text-xl" className="fa-xmark text-xl" /></button>
                        </div>
                        ...
                        <div className="flex gap-2 mb-6 border-b border-white/10 pb-2">
                            ...
                            ><Icon name="fa-box mr-2" className="fa-box mr-2" />{t(config.language || 'vi', 'forge.catalystModal.items')}</button>
                            <button 
                            ...
                            ><Icon name="fa-skull mr-2" className="fa-skull mr-2" />{t(config.language || 'vi', 'forge.catalystModal.cards')}</button>
                        </div>
                            ...
                            <>
                                <div className="space-y-4 mb-6 overflow-y-auto pr-2 custom-scrollbar flex-1 min-h-0">
                                    <label className={`flex flex-row items-center gap-4 p-3 rounded-xl border cursor-pointer transition-all ${tempUpgradeId === 'quantumDust' ? 'bg-cinematic-gold/10 border-cinematic-gold/50' : 'bg-black/40 border-white/10 hover:border-cinematic-gold/30'}`}>
                                        ...
                                        <div className="w-10 h-10 rounded-full bg-cinematic-gold/20 flex items-center justify-center shrink-0">
                                            <Icon name="fa-meteor text-cinematic-gold text-xl" className="fa-meteor text-cinematic-gold text-xl" />
                                        </div>
                                        <div className="flex-1">
                                            <div className="text-sm font-bold text-white mb-0.5">Quantum Dust</div>
                                            <div className="text-[10px] text-cinematic-gold/80">{t(config.language || 'vi', 'forge.catalystModal.itemDesc')}</div>
                                            <div className="text-[10px] text-white/50 mt-1">{t(config.language || 'vi', 'forge.catalystModal.current')}: {inventory.quantumDust || 0} | {t(config.language || 'vi', 'forge.catalystModal.available')}: {getMaxUpgradeAmount('quantumDust')}</div>
                                        </div>
                                    </label>
                                    ...
                                    {matsAvailable.map(mat => (
                                        <label key={mat} className={`flex flex-row items-center gap-4 p-3 rounded-xl border cursor-pointer transition-all ${tempUpgradeId === mat ? 'bg-cinematic-gold/10 border-cinematic-gold/50' : 'bg-black/40 border-white/10 hover:border-cinematic-gold/30'}`}>
                                            ...
                                            <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center border border-white/10 shrink-0">
                                                <Icon name="fa-gem text-cinematic-cyan text-lg" className="fa-gem text-cinematic-cyan text-lg" />
                                            </div>
                                            <div className="flex-1">
                                                <div className="text-sm font-bold text-white mb-0.5">{mat}</div>
                                                <div className="text-[10px] text-cinematic-cyan/80">Can thiệp cấu trúc Gen (100%)</div>
                                                <div className="text-[10px] text-white/50 mt-1">{t(config.language || 'vi', 'forge.catalystModal.current')}: {inventory.materials[mat]} | {t(config.language || 'vi', 'forge.catalystModal.available')}: {getMaxUpgradeAmount(mat)}</div>
                                            </div>
                                        </label>
                                    ))}
                                    {matsAvailable.length === 0 && (
                                        <div className="text-center p-4 border border-dashed border-white/10 rounded-xl bg-black/20">
                                            <p className="text-[10px] text-white/40 uppercase tracking-widest">{t(config.language || 'vi', 'forge.catalystModal.noMats')}</p>
                                        </div>
                                    )}
                                </div>
                                ...
                                {tempUpgradeId && (
                                    <div className="bg-black/40 border border-cinematic-gold/20 p-4 rounded-xl mb-6">
                                        <div className="text-[10px] uppercase text-cinematic-gold tracking-widest mb-3 text-center">{t(config.language || 'vi', 'forge.catalystModal.qty')}</div>
                                        <div className="flex items-center justify-center gap-4">
                                            ...
                                            ><Icon name="fa-minus" className="fa-minus" /></button>
                                            ...
                                            <div className="w-24 text-center">
                                                <input 
                                                    ...
                                                    min={0}
                                                />
                                                <div className="text-[8px] text-white/40 mt-1">{t(config.language || 'vi', 'forge.catalystModal.max')}: {tempUpgradeId ? getMaxUpgradeAmount(tempUpgradeId) : 0}</div>
                                            </div>
                                            ...
                                            <button 
                                                ...
                                                className="w-10 h-10 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-white flex items-center justify-center"
                                            ><Icon name="fa-plus" className="fa-plus" /></button>
                                        </div>
                                    </div>
                                )}
                            </>
                        ...
                        {activeUpgradeTab === 'card' && (
                            <div className="flex-1 min-h-0 flex flex-col mb-6">
                                <div className="text-[9px] text-red-400 bg-red-900/20 border border-red-900/50 p-2 rounded-lg mb-4 uppercase tracking-widest text-center leading-relaxed">
                                    <Icon name="fa-triangle-exclamation mr-1" className="fa-triangle-exclamation mr-1" /> <span dangerouslySetInnerHTML={{ __html: t(config.language || 'vi', 'forge.catalystModal.cardWarning') }} />
                                </div>
                                <div className="space-y-3 overflow-y-auto pr-2 custom-scrollbar">
                                    {availableCardsForSacrifice.length > 0 ? availableCardsForSacrifice.sort((a,b) => getRankIndex(b.cardClass) - getRankIndex(a.cardClass)).map(c => (
                                        ...
                                        <div className="text-center p-4 border border-dashed border-white/10 rounded-xl bg-black/20">
                                            <p className="text-[10px] text-white/40 uppercase tracking-widest">{t(config.language || 'vi', 'forge.catalystModal.noCards')}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="flex gap-3 mt-auto shrink-0">
                            <button onClick={() => setShowUpgradeModal(false)} className="flex-1 bg-white/5 hover:bg-white/10 text-white py-3 rounded-xl font-bold transition-colors uppercase tracking-widest text-[10px]">{t(config.language || 'vi', 'forge.catalystModal.btnCancel')}</button>
                            {activeUpgradeTab === 'item' ? (
                                <button 
                                ...
                                >{t(config.language || 'vi', 'forge.catalystModal.btnConfirmItem')}</button>
                            ) : (
                                <button 
                                    onClick={confirmUpgradeSelect} 
                                ...
                                ><Icon name="fa-fire mr-1" className="fa-fire mr-1" /> {t(config.language || 'vi', 'forge.catalystModal.btnConfirmCard')}</button>
                            )}
                        </div>
                    </div>
                </div>
            ...
            <Dialog
                isOpen={showConfirm}
                title={t(config.language || 'vi', 'forge.confirmTitle')}
                message={t(config.language || 'vi', 'forge.confirmMessage', { cost: dynamicCost })}
                type="confirm"
