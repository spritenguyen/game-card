...
export const OverclockView: React.FC<Props> = ({ config, currency, modifyCurrency, inventory, cards, modifyInventory, onUpdateCard, onError, onAlert, isGlobalProcessing, setGlobalProcessing }) => {
    ...
    const handleSelectCard = (id: string) => {
        ...
        if (selectorTarget.type === 'target') {
            if (c.cardClass !== 'UR') return onError(t('overclock.errorUrOnly'));
            setTargetSlot(c);
            ...
            if (c.cardClass !== reqRank) return onError(t('overclock.errorRankReq', { rank: reqRank }));
            if (c.faction !== targetSlot?.faction) return onError(t('overclock.errorFactionReq', { faction: targetSlot?.faction || '' }));
            if (targetSlot?.id === c.id) return onError(t('overclock.errorTargetAsSac'));
            if (sacrificeSlots.some(s => s?.id === c.id)) return onError(t('overclock.errorAlreadyUsed'));
    ...
    const executeOverclock = async () => {
        ...
        if (currency < cost.dc) return onError(t('overclock.errorCredits', { dc: cost.dc }));
        ...
        try {
            if (currency < cost.dc) {
                ...
                return onError(t('overclock.declined'));
            }
        ...
        <div className="w-full flex flex-col items-center animate-fade-in pb-12">
            ...
            <div className="text-center mb-10 max-w-2xl mx-auto relative z-10">
                <h1 className="text-2xl sm:text-4xl font-serif text-transparent bg-clip-text bg-gradient-to-br from-white via-purple-300 to-purple-500 uppercase tracking-[0.3em] mb-4">
                    <Icon name="fa-bolt mr-3 text-purple-400" className="fa-bolt mr-3 text-purple-400" />{t('overclock.title')}
                </h1>
                <p className="text-[10px] sm:text-[11px] text-zinc-400 font-mono tracking-widest uppercase">
                    {t('overclock.subtitle')}
                </p>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-8 lg:gap-12 w-full items-start justify-center relative z-10">
                        ...
                        <div className="flex flex-col items-center">
                            ...
                            >
                                {targetSlot ? (
                                    ...
                                    <div className="text-center px-4 flex flex-col items-center">
                                        <Icon name="fa-microchip text-4xl sm:text-5xl text-zinc-700 mb-4" className="fa-microchip text-4xl sm:text-5xl text-zinc-700 mb-4" />
                                        <p className="text-[10px] sm:text-xs text-zinc-500 uppercase font-mono tracking-[0.2em]">{t('overclock.selectUr')}</p>
                                    </div>
                                )}
                            </div>
                            {targetSlot && (
                                <button onClick={() => !isGlobalProcessing && setTargetSlot(null)} className="mt-4 text-[9px] sm:text-[10px] bg-red-950/30 text-red-400 border border-red-900/50 px-4 py-1.5 rounded-full hover:bg-red-900 hover:text-white transition-all uppercase font-mono tracking-widest flex items-center shadow-lg">
                                    <Icon name="fa-xmark mr-2" className="fa-xmark mr-2" /> {t('overclock.unlink')}
                                </button>
                            )}
                        </div>
                         ...
                         <div className="flex justify-center items-center py-2 h-10 w-full relative">
                            ...
                            <div className="bg-black border border-white/10 rounded-full w-8 h-8 flex items-center justify-center relative z-10 shadow-lg">
                                <Icon name="fa-plus text-sm text-zinc-500" className="fa-plus text-sm text-zinc-500" />
                            </div>
                         </div>
                         ...
                         <div className="w-full flex justify-center">
                            {targetSlot && !isMax ? (
                                            ...
                                            >
                                                {sCard ? (
                                                    <>
                                                        ...
                                                        >
                                                            <Icon name="fa-xmark text-[8px]" className="fa-xmark text-[8px]" />
                                                        </button>
                                                    </>
                                                ) : (
                                                    <div className="text-center px-1 flex flex-col items-center">
                                                        <Icon name="fa-skull text-xl sm:text-2xl text-zinc-800 mb-2" className="fa-skull text-xl sm:text-2xl text-zinc-800 mb-2" />
                                                        <p className="text-[7px] sm:text-[8px] text-zinc-500 uppercase tracking-[0.2em] font-mono font-bold">{t('overclock.req')} {req.rank}</p>
                                                    </div>
                                                )}
                                            </div>
                                        );
                                ...
                                <div className="w-full max-w-[280px] h-28 sm:h-36 rounded-xl flex flex-col items-center justify-center border border-dashed border-white/5 ring-1 ring-white/5 bg-black/30">
                                    <Icon name="fa-lock text-2xl text-zinc-800 mb-3" className="fa-lock text-2xl text-zinc-800 mb-3" />
                                    <p className="text-[9px] sm:text-[10px] text-zinc-600 uppercase tracking-[0.2em] font-mono text-center px-4">{t('overclock.insertUrFirst')}</p>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="w-full md:w-1/2 flex flex-col gap-5 sm:gap-6">
                    <div className="bg-black/60 border border-white/5 ring-1 ring-white/5 rounded-2xl p-5 sm:p-6 shadow-inner relative overflow-hidden backdrop-blur-sm">
                        <div className="absolute top-0 right-0 p-4 opacity-[0.03]"><Icon name="fa-chart-line text-8xl" className="fa-chart-line text-8xl" /></div>
                        <h3 className="text-white text-xs sm:text-sm font-serif tracking-[0.2em] uppercase mb-5 border-b border-white/5 pb-3 flex items-center justify-between">
                            <span><Icon name="fa-microchip text-purple-400 mr-2" className="fa-microchip text-purple-400 mr-2" /> {t('overclock.metrics')}</span>
                        ...
                        <ul className="text-[10px] sm:text-xs text-zinc-400 flex flex-col gap-3 font-mono tracking-widest relative z-10 w-full">
                            <li className="flex justify-between items-center w-full bg-white/5 p-2 rounded-lg border border-white/5">
                                <span className="text-zinc-500 uppercase">{t('overclock.targetNode')}</span>
                                <span className="text-purple-400 font-bold uppercase">{t('overclock.protocol')}</span>
                            </li>
                            {targetSlot && !isMax && (
                                <li className="flex justify-between items-start w-full bg-white/5 p-2 rounded-lg border border-white/5">
                                    <span className="text-zinc-500 uppercase pt-0.5">{t('overclock.sacReqs')}</span>
                            ...
                            <li className="flex justify-between items-center w-full bg-white/5 p-2 rounded-lg border border-white/5">
                                <span className="text-zinc-500 uppercase">{t('overclock.yield')}</span>
                                <span className="text-green-400 font-bold">+10% {t('overclock.baseStats')}</span>
                            </li>
                             <li className="flex justify-between items-center w-full bg-white/5 p-2 rounded-lg border border-white/5">
                                <span className="text-zinc-500 uppercase">{t('overclock.currentLvl')}</span>
                                <span className="text-white">{currentLvl > 0 ? `+${currentLvl}` : t('overclock.baseline')}</span>
                            </li>
                            {targetSlot && !isMax && (
                                <li className="flex justify-between items-center w-full bg-cinematic-cyan/5 p-2 rounded-lg border border-cinematic-cyan/10 mt-2">
                                    <span className="text-cinematic-cyan uppercase">{t('overclock.estOutput')}</span>
                                    <span className="text-cinematic-cyan font-bold">+{(currentLvl + 1) * 10}% {t('overclock.total')}</span>
                                </li>
                            )}
                            {targetSlot && !isMax && (
                                <li className="flex justify-between items-center w-full bg-red-950/20 p-2 rounded-lg border border-red-900/30">
                                    <span className="text-red-400 flex items-center gap-1.5 uppercase"><Icon name="fa-biohazard" className="fa-biohazard" /> {t('overclock.successRate')}</span>
                            ...
                            {isMax && (
                                <li className="flex justify-center items-center w-full bg-cinematic-gold/10 p-3 rounded-lg border border-cinematic-gold/20 mt-2">
                                    <span className="text-cinematic-gold font-bold uppercase tracking-[0.3em]">{t('overclock.maxProtocol')}</span>
                                </li>
                            )}
                        </ul>
                    </div>
                    ...
                    <div className="bg-black/60 border border-white/5 ring-1 ring-white/5 rounded-2xl p-5 sm:p-6 shadow-inner relative overflow-hidden backdrop-blur-sm">
                        <div className="absolute top-0 right-0 p-4 opacity-[0.03]"><Icon name="fa-server text-8xl" className="fa-server text-8xl" /></div>
                       <h3 className="text-white text-xs sm:text-sm font-serif tracking-[0.2em] uppercase mb-4 border-b border-white/5 pb-3 flex items-center">
                             <Icon name="fa-coins text-cinematic-gold mr-2" className="fa-coins text-cinematic-gold mr-2" /> {t('overclock.resourceDemand')}
                        </h3>
                        <div className="flex flex-col gap-3 font-mono text-[10px] sm:text-xs tracking-widest relative z-10 w-full">
                            <div className="flex items-center justify-between p-3 sm:p-4 bg-white/5 rounded-xl border border-white/5 hover:border-cinematic-gold/20 transition-colors">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-zinc-400 uppercase">{t('overclock.dataCredit')}</span>
                                </div>
                                <div className="flex flex-col items-end">
                                   <span className={`font-bold text-sm ${currency >= cost.dc ? 'text-cinematic-gold' : 'text-red-500'}`}>
                                       {isMax ? '---' : cost.dc.toLocaleString()}
                                   </span>
                                   <span className="text-[8px] sm:text-[9px] text-zinc-600 mt-1">{t('overclock.avail')} {currency.toLocaleString()}</span>
                                </div>
                            </div>
                            <div className="flex items-center justify-between p-3 sm:p-4 bg-white/5 rounded-xl border border-white/5 hover:border-cinematic-cyan/20 transition-colors">
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-full bg-cinematic-cyan/10 flex items-center justify-center border border-cinematic-cyan/20 text-cinematic-cyan">
                                        <Icon name="fa-atom" className="fa-atom" />
                                    </div>
                                    <span className="text-zinc-400 uppercase">{t('overclock.quantumDust')}</span>
                                </div>
                                 <div className="flex flex-col items-end">
                                   ...
                                   <span className="text-[8px] sm:text-[9px] text-zinc-600 mt-1">{t('overclock.avail')} {(inventory.quantumDust || 0).toLocaleString()}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <button 
                     ...
                     >
                         ...
                         {isGlobalProcessing ? (
                             <><Icon name="fa-circle-notch fa-spin text-xl" className="fa-circle-notch fa-spin text-xl" /> {t('overclock.initializing')}</>
                         ) : isMax ? (
                             <><Icon name="fa-lock text-xl opacity-50" className="fa-lock text-xl opacity-50" /> {t('overclock.maxReached')}</>
                         ) : (
                             <><Icon name="fa-bolt text-xl group-hover:scale-125 transition-transform" className="fa-bolt text-xl group-hover:scale-125 transition-transform" /> {t('overclock.initialize')}</>
                         )}
                     </button>
                </div>
            </div>
                    ...
                    <div className="relative z-10 w-full max-w-4xl max-h-[80vh] flex flex-col bg-cinematic-900 border border-white/10 rounded-2xl p-6 shadow-2xl">
                        <div className="flex justify-between items-center mb-6 shrink-0">
                            <h2 className="text-white font-bold text-lg sm:text-xl uppercase tracking-widest font-serif flex items-center gap-3">
                                {t('overclock.selectCard')} 
                                <span className={selectorTarget.type === 'target' ? "text-cinematic-gold" : "text-red-500"}>
                                    {selectorTarget.type === 'target' ? t('overclock.targetUr') : t('overclock.targetSacrifice', { rank: selectorTarget.index !== undefined ? requiredSacrifices[selectorTarget.index].rank : 'SSR/SR', faction: targetSlot?.faction || '' })}
                                </span>
                            </h2>
                            <button onClick={() => setSelectorTarget(null)} className="w-8 h-8 flex items-center justify-center text-white/50 hover:text-white transition-colors">
                                <Icon name="fa-xmark text-xl" className="fa-xmark text-xl" />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto min-h-[300px] pr-2 custom-scrollbar">
                            ...
                            {cards.filter(c => selectorTarget.type === 'target' ? c.cardClass === 'UR' : (selectorTarget.index !== undefined ? c.cardClass === requiredSacrifices[selectorTarget.index].rank && c.faction === targetSlot?.faction : false)).length === 0 && (
                                <div className="py-12 text-center text-zinc-500 font-mono">
                                    {t('overclock.notFound', { type: selectorTarget.type === 'target' ? 'UR' : (selectorTarget.index !== undefined ? `${requiredSacrifices[selectorTarget.index].rank}` : '') })}
                                </div>
                            )}
                        </div>
            ...
            <Dialog
                isOpen={showConfirm}
                title={t('overclock.dialogTitle')}
                message={t('overclock.dialogMsg', { dc: cost.dc, dust: cost.dust, count: requiredSacrifices.length, rate: successRate })}
                type="confirm"
